"""Freeze one SM2311 source request, then use the existing verifier."""
import hashlib
import json
import subprocess
import sys
...
with SessionLocal() as session, readonly_transaction(session, timeout_seconds=300):
    before = catalog_revision(session, ("sm",), cutoff.date(), ("1d", "1w"))
    result = _diagnose(build_market_data_service(session), {"symbol": "sm", "contract": "SM2311", "through": "2023-10-10"}, cutoff)
    after = catalog_revision(session, ("sm",), cutoff.date(), ("1d", "1w"))
    if before != after:
        raise RuntimeError("CATALOG_REVISION_CHANGED")
    weeks = [week for week in result["conflict_weeks"] if week["trading_day"] == "2023-09-01"]
    ...
    body = {"schema_version": "newow_weekly_conflict_diagnosis_v2", "status": "diagnosed", "catalog_revision_stable": True,
            ...
            "input_sha256": hashlib.sha256(b"SM2311 2023-09-01 targeted conflict").hexdigest(), "contracts": [result]}
plan = build_source_plan(body)
out = Path(__file__).parent
(out / "sm2311-20230901-source-diagnosis.json").write_text(json.dumps(body, ensure_ascii=False, indent=2) + "\n")
(out / "sm2311-20230901-source-plan.json").write_text(json.dumps(plan, ensure_ascii=False, indent=2) + "\n")
