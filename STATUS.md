...
- v1.9.8 已 `RELEASED`，但 Runtime promotion 因 active60 全部 `STALE_INPUT` fail-closed 回滚，当前不是 `RUNTIME_READY`。共同根因已在 `fbf468cba` 修复并通过完整验证；后续仍需新的 Release 批准，再需独立 Runtime promotion 授权，不以 develop 提交或测试替代两个 Gate。
