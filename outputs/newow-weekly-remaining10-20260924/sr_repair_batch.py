"""Prepare or apply the two pinned SR W1 no-trade repairs from existing Canonical D1.
...
def _prepare_target(
    ...
    daily_key = DatasetKey("contract", "sr", contract, "1d")
    weekly_key = DatasetKey("contract", "sr", contract, "1w")
    ...
    if len(weekly_partitions) != 1:
        raise SRRepairError("W1_PARTITION_IDENTITY_INVALID")
    ...
    if len(matches) != 1:
        raise SRRepairError("W1_TARGET_INVALID")
    old = matches[0]
    ...
    expected = tuple(
        point for point in mds.expected_contract_replay_endpoints(
            symbol="sr", contract=contract, frequency=daily_key.frequency,
    ...
    if not expected or tuple((bar.bar_end, bar.trading_day) for bar in daily) != expected:
        raise SRRepairError("D1_COMPLETE_WEEK_INVALID")
    fact = catalog.contract_fact("sr", contract)
    if any(not fact.listed_date <= bar.trading_day < fact.expired_date for bar in daily):
        raise SRRepairError("D1_LIFECYCLE_INVALID")
    ...
    if old == new or old.trading_day != new.trading_day:
        raise SRRepairError("W1_TARGET_DIFF_INVALID")
    ...
    if len(candidate) != len(month_bars) or sum(a != b for a, b in zip(candidate, month_bars)) != 1:
        raise SRRepairError("W1_MONTH_DIFF_INVALID")
...
def prepare(session: Session, root: Path, root_sha256: str) -> tuple[dict[str, Any], tuple[tuple[CanonicalBar, ...], ...]]:
    ...
    revision = catalog_revision(session, ("sr",), _CUTOFF, ("1d", "1w"))
    results = tuple(
        _prepare_target(catalog, store, mds, root, *target) for target in _TARGETS
    )
    if catalog_revision(session, ("sr",), _CUTOFF, ("1d", "1w")) != revision:
        raise SRRepairError("CATALOG_REVISION_MOVED")
    return ({
        "schema_version": "sr_weekly_no_trade_prepare_v1",
    ...
    try:
        ...
        for record, candidate in zip(packet["targets"], candidates, strict=True):
            ...
            if published.parquet_path.relative_to(root).as_posix() != record["candidate_w1_partition_uri"]:
                raise SRRepairError("CANDIDATE_HASH_MISMATCH")
            ...
            MarketDataService(catalog, store).query_contract_weekly_replay_quality(
                symbol="sr", contract=contract, through=day,
        ...
        try:
            ...
            raise SRRepairError("COMMIT_OUTCOME_UNKNOWN") from exc
        committed = True
    ...
    for record in packet["targets"]:
        for source in record["d1_preimages"]:
            ...
            if relative.is_absolute() or ".." in relative.parts:
                raise SRRepairError("D1_PREIMAGE_URI_INVALID")
            ...
            if (source_path.is_symlink() or not source_path.resolve().is_relative_to(root.resolve())
                ...
                raise SRRepairError("D1_PREIMAGE_MOVED")
        day = date.fromisoformat(record["week_end"])
        key = DatasetKey("contract", "sr", record["contract"], "1w")
        selected = tuple(
        ...
        if uri == record["old_w1_partition_uri"] and digest == record["old_w1_partition_sha256"]:
            ...
            bars, _ = MarketDataService(catalog, store).query_contract_weekly_replay_quality(
                symbol="sr", contract=record["contract"], through=day,
            ...
            if not any(bar.trading_day == day for bar in bars):
                raise SRRepairError("CANDIDATE_READBACK_INVALID")
    ...
    try:
        with Session(engine, autoflush=False) as session:
            if args.mode == "prepare":
                ...
                output = Path.cwd() / "outputs/newow-weekly-remaining10-20260924/sr-prepare.json"
                ...
                if _sha(content) != args.expected_prepared_sha256:
                    raise SRRepairError("PREPARED_HASH_MISMATCH")
                ...
                if args.mode == "inspect":
                    ...
                    raise SRRepairError("ACTIVE_POINTERS_MIXED_OR_UNKNOWN")
                else:
                    ...
                    if state["status"] != "candidate":
                        raise SRRepairError("COMMIT_READBACK_FAILED")
...
if __name__ == "__main__":
    ...
    except SRRepairError as exc:
        print(json.dumps({"status": "failed", "error_code": str(exc)}))
        raise SystemExit(1) from None
    except Exception:
        print(json.dumps({"status": "failed", "error_code": "SR_REPAIR_FAILED"}))
        raise SystemExit(1) from None
