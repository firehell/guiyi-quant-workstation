...
SuBing Strategy V1 Stage 1 是与上述三类当前事实分离的 Historical Strategy Projection：只支持 active 单品种 `actual_dominant + 15m + since/through`，在每个 rank1 物理合约段内从真实段起点复算 Factor/Lifecycle/Strategy。四种 `ENTRY_CONFIRMED` 来源均可按 Daily Watch V2 上一共同交易日 D1/60m 方向建仓；普通动作在下一根同物理合约 15m Bar open 模拟生效，只有 EMA21、上一根 15m 极值、绑定 Pivot 和 MACD 高低位反向交叉四类退出。不加减仓、不反手、不做同 Bar 重建仓、不跨物理段；只有请求覆盖到权威段末时才以旧段最后一根 15m close 清仓，普通 `through` 不冒充 terminal。Action/Episode 、“参考变动”与 Web Marker/记录都是历史因果投影和模拟动作，非实际成交；缓存只能位于校验后 observation base 的 `cache/subing-strategy-v1/`，可删除、非权威且失败不得改变结果。该能力不创建账户、订单、保证金、费用/滑点、资金曲线、通用回测平台、Alert Rule 或 Stage 2 completed-Live。
