...
- SuBing 全历史效果 API、active60 cache warm 与 Web 底部面板都复用 Historical Projection；批次窗口先冻结，按产品以完整 engine/source identity 原子发布并读回可删除快照。盘后只在 Canonical 更新和 Live reconciliation 成功后以该成功交易日刷新；派生失败进入 after-market schema v3 的 degraded stage，不回滚 Canonical、不发送通知、不重试。
