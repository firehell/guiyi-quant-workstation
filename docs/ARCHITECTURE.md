...
- `/market/chart?view=newow` 的趋势、震荡、主升浪 × `1w/1d/60m` 消费仍待 P5 实现；P4 只提供新的 sectioned read-only API。既有 `view=trend` 与 `GET /api/v1/market/newow/trend-detail` 保持固定 `actual_dominant + 1d` 兼容语义，并可经薄适配复用同一趋势公式，不保留第二套算法。HTDY、SuBing 与 Free 的读取和 Marker authority 不变。
