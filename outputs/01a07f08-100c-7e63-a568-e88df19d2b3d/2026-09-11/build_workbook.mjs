...
for(const s of [summary,detail,content]) {s.showGridLines=false;s.getRange('A1:AP250').format.font={name:'Arial',size:11,color:bodyColor};s.getRange('A1:AP250').format.verticalAlignment='center';}
...
detail.getRange('A5').values=[['北京时间。基准为记录的发送尝试时间之后首个分钟收盘价；15/30/60m 为随后交易分钟。空白终点价需看状态：数据不足或未满，均不计作0。右侧AL:AN保留9月10日快照结果，AO:AP为重叠窗口分组。']];
...
detail.getRange('AL6:AN6').values=[['9/10快照15m','9/10快照30m','9/10快照60m']];detail.getRange(`AL7:AN${end}`).values=a.records.map(r=>[numeric(r.previous_return_15),numeric(r.previous_return_30),numeric(r.previous_return_60)]);pctColor(detail,`AL7:AN${end}`);detail.getRange(`AL6:AN${end}`).format.columnWidth=18;detail.getRange('AO6:AP6').values=[['60m窗口组','组首条']];detail.getRange(`AO7:AP${end}`).values=a.records.map(r=>[r.cluster_id,r.cluster_first]);detail.getRange(`AO6:AP${end}`).format.columnWidth=16;nativeTable(detail,`A6:AP${end}`,'PushPerformance');header(detail,'A6:AP6');
...
summary.getRange('A4').values=[[`当日新增 ${a.today_total.count} 条；本周 ${a.total.count} 条。预警仍停在9月10日10:15。今天0条新增不能解释为正常无信号。`]];
summary.getRange('A5').values=[[`今日60品种缺有效Session及当日rank1映射；本周当前60m可重算 ${a.total.n60} 条，${a.total.missing60} 条数据不足。`]];
...
section('当日预警表现',a.today_strategy_summary,'strategy',a.today);
...
detail.getRange(`AG6:AK${end}`).format.columnWidth=18;detail.getRange(`W6:W${end}`).format.columnWidth=55;
w.recalculate();
...
if(!base)throw Error('probe anchor unavailable');detail.getRange('N7').values=[[base*(1+side*.01)]];if(Math.abs(Number(detail.getRange('O7').values[0][0])-.01)>1e-10)throw Error('recalculation failed');detail.getRange('N7').values=[[original]];w.recalculate();
...
for(const [sheetName,range,file] of [['复盘汇总','A1:N17','summary-preview.png'],['逐条表现','R6:X12','detail-preview.png'],['推送内容','A6:H9','contents-preview.png']]){const b=await w.render({sheetName,range,scale:1.3,format:'png'});await fs.writeFile(`${dir}${file}`,new Uint8Array(await b.arrayBuffer()));}
