...
P4 候选的局部实现提交为 `098a6b15c`（section 编排）、`7ba332325`（typed read-only API）、`c96430dc0`（缓存/资源/来源收口与已测 Hint 索引优化）、`22b121fc1`（首轮 Review 合同修复）和 `44687e475`（扩展性能入口与 active 架构关系）。当前有效的影响范围证据为：P2/P3/P4/旧 D1 相关 pytest `484 passed, 1 skipped`，修改源定向 mypy clean，修改文件 ruff clean。`44687e475` 上隔离 fake MDS 的 nearest-rank P95 为：4001根60m冷请求30次 `177.402ms`、逐事实重验后缓存命中30次 `72.858ms`、8001根压力30次 `323.351ms`；601根阶段样本的读取+验证 `4.564ms`、replay `13.357ms`、ReferenceProjection `0.111ms`、reference总计 `21.095ms`、comparator总计 `23.645ms`，90根历史修订重验30次 `8.408ms`、序列化30次 `2.882ms`，响应 `289034 bytes`、RSS high-water `221429760 bytes`。排队、取消、最后消费者语义使用确定性并发测试而非时间P95。该性能证据只覆盖后端 fake-MDS 阶段，不是浏览器、真实 MDS 或真实工作站验收；最终候选若相关源码/fixture变化必须使相应证据失效并补跑。
