...
新增WR20/MA120趋势转折副图尚未实现。CDV2 1.2.0公开内核哈希与9月10日相同，归一仍未迁移新版解释规则；
新增外围诊股仓位同步路径已记录。三策略内核已有，正式产品仍仅D1，跨频解释未开放。

以下内容完整保留为9月9～10日历史证据，表格中的“当前”和P1/P2指当时审计阶段，不作为9月18日实现清单。
最新实现清单、截图、源码哈希及剩余验证范围统一见上方入口；不重复计算旧golden为本轮新通过。

## 2026-09-09～10 历史核对

核对日期：2026-09-09～2026-09-10。早期证据来自 Chrome 可见页面和 App 更新截图；随后完成趋势周线、震荡60分钟、主升浪60分钟、目标/吸筹日周及综合解释 v2 的固定快照验证。趋势、主升浪和目标/吸筹在限定范围内通过；震荡图表的同根重建差异按 owner 决定保留并退出当前待办；综合解释确认是新计算合同，归一当前版本尚未实现，见各专节。
最初代码核对基线为 `a31962af3a5bf17fefc32ea2dc7bc5fe7625795e`；综合解释对照使用 `65d774f81256d0ea24c6902acb5ec7c2d07cfdf8`。工程交付状态见 [STATUS](../../STATUS.md)，产品语义见 [Newow OpenSpec](../../openspec/specs/newow-product-reference-trading/spec.md)。

## 结论与范围

保留现有三策略九组合、统一行情入口和只读参考交易。本轮已补空仓等待卡片和周期日期呈现，并完成主升浪60分钟、目标/吸筹日周及综合解释 v2 的固定同输入比较。综合解释的五项、R2/R3、MM1-MM4 和计龄规则已确认，但归一现有四项/13格合同不一致；需要新版本设计后再实现。不重建震荡60分钟引擎，不增加清仓回填链路。收益曲线、年化/回撤与嵌套路径是独立候选，须先冻结口径。
...
| N09 | 共振 R2、错配、已清 N 根与分项解释 | O06及9月10日固定快照 | 三个真实输入、两个DOM与MM1-MM4派生见证已确认五项、R2/R3、3根门槛和计龄；归一现有13格/四项封顶合同3/3不一致，且牛哇非零certExtra会造成五项与总分不闭合 | P1 新建版本化解释合同；实现R0-R4、MM1-MM4、计龄和五项，并显式处理certExtra；不覆盖旧版本、不引入隐藏Gate |
...
### P1：综合解释进入版本设计，其余项目保留证据 Gate

1. **综合解释**：同快照输入输出表已经建立，并确认是规则变化。下一步先写版本化解释合同和实现计划：新增R0-R4、MM1-MM4、`mismatchBars`、五项及双轴仓位，明确`certExtra`的显示/禁用选择；保留旧版本可重放，不把评分变成策略隐藏Gate。获得计划批准后才改active实现。
