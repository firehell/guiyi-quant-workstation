...
后续 merge 由 Codex 编排层执行。`release-flow.sh prepare/publish/tag` 仍只在用户批准exact packet、
current/target/rollback SHA、clean worktree与远端refs精确匹配时逐项执行；task自动集成不授权
release、tag 或 Runtime。
