...
| Develop 中工作 | `develop` 上的 Newow futures validation、SuBing detail workspace 与 physical-contract warm-up 都是未发布的开发工作。warm-up 已完成 source rebase 与 Task 1–6 提交；仍须处理独立 Review 发现、完成 fresh verification 并通过独立 Review 后才可能形成 RC；代码存在不等于 release、数据 apply、Runtime promotion 或自然验收。 |
