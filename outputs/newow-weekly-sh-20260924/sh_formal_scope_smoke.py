"""Read-only formal SH W1 API smoke on development code and actual Catalog."""
...
try:
    with TestClient(app) as client:
        ...
        if (cap.status_code != 200
                or capability["schema_version"] != "newow_product_capabilities_v21"
                or len(capability["weekly_products"]) != 59
                or "sh" not in capability["weekly_products"]
                or any(p in capability["weekly_products"] for p in ("sm",))):
        ...
        for strategy, statuses in expected.items():
            for section, expected_status in statuses.items():
                params = {
                    "product": "sh", "strategy": strategy, "frequency": "1w", "section": section,
        ...
        closed = client.get(
            ...
            params={"product": "sm", "strategy": "trend", "frequency": "1w", "section": "chart"},
        )
        ...
        result = {"status": "passed", "scope_count": 59, "schema_version": capability["schema_version"],
                  "sections": sections, "sm_still_closed": True, "database_mode": "read_only"}
        (Path(__file__).parent / "sh-formal-scope-smoke.json").write_text(
