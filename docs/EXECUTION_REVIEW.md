...
- 自动 roll reconcile 默认关闭，只能通过独立 Gate 激活；它不是 `AfterMarketUpdater` 内部职责，
  不 replay/backfill 历史。`record_executed` 必须逐次读取 Gate：marker 为 `disabled` 或 `invalid`
  时一律返回 `ROLL_RECONCILIATION_REQUIRED`，且不得创建 `DOMINANT_ROLL`；只有精确为 `enabled`
  才允许调用 reconcile 并可能形成 `DOMINANT_ROLL`。当前激活状态只看 `STATUS.md`。
