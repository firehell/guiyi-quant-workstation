 ...
 with q.Session(bind=conn,autoflush=False) as s:
  ...
  for e in sorted(source['events'],key=lambda e:e['bar_end']):
   try:
    ...
    if e['rule_code']=='htdy_original_15m':
     expected=q.build_market_data_service(s).expected_contract_replay_endpoints(symbol=e['symbol'],contract=e['contract'],frequency=e['frequency'],trading_day=day,cutoff=t,since=day-timedelta(days=14))
     assert tuple((b.bar_end,b.trading_day) for b in window.bars[-32:])==expected[-32:],'FORMULA_PREFIX_COVERAGE_MISSING'
