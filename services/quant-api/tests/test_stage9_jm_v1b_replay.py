...
def test_replay_dry_run_finds_candidate_without_writing(tmp_path: Path) -> None:
    ...
    with TestingSessionLocal() as session:
        ...
        assert result["would_write_signal_event"] is False
...
def test_replay_run_write_flag_is_preview_only_and_idempotently_writes_nothing(tmp_path: Path) -> None:
    TestingSessionLocal = _session_factory(tmp_path)
...
def test_replay_preview_result_does_not_expose_sensitive_words_or_write(tmp_path: Path) -> None:
    TestingSessionLocal = _session_factory(tmp_path)
