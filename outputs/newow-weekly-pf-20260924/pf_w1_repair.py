"""Exact PF W1 missing-week and receipt-backed turnover repair."""
...
PREPARED = HERE / "pf-w1-prepare.json"
...
def load_targets():
    ...
    if (audit["products"] != ["pf"] or audit["revision_stable"] is not True
            or audit["provider_requests"] != 0 or audit["writes"] != 0
            or len(audit["contracts"]) != 40
            or any(row["status"] != "SCANNED" for row in audit["contracts"])):
    ...
    if (dict(counts) != {"W1_MISSING_D1_COMPLETE": 1054,
                         "D1_W1_VALUE_CONFLICT": 26,
                         "STRICT_NO_TRADE_AGGREGATION": 138,
                         "PROVEN_QUALITY_INTERRUPTION": 84}
            or len(targets) != 39 or "PF2611" in targets):
        raise ValueError("ISSUE_COUNTS_MOVED")
    receipts = json.loads(RECEIPTS.read_bytes())
    evidence = {}
    for row in receipts["rows"]:
        if row["symbol"] != "pf" or not row["all_daily_receipted"]:
            continue
            ...
            for receipt_path in part["matching_receipts"]:
                ...
                if (receipt["status"] != "passed" or receipt["contract"] != row["contract"]
                        ...
                        or not any(item["year"] == part["year"] and item["month"] == part["month"]
                                   and item["dataset"] == ["contract", "pf", row["contract"], "1d"]
...
def build_packet(session, root: Path, config_identity):
    ...
    revision = catalog_revision(session, ("pf",), CUTOFF, ("1d", "1w"))
    ...
    for contract in sorted(targets):
        d1_key = DatasetKey("contract", "pf", contract, "1d")
        w1_key = DatasetKey("contract", "pf", contract, "1w")
        ...
        for issue in targets[contract]:
            ...
            weekly_points = [point for point in mds.expected_contract_replay_endpoints(
                symbol="pf", contract=contract, frequency=w1_key.frequency,
            ...
            expected = tuple(point for point in mds.expected_contract_replay_endpoints(
                symbol="pf", contract=contract, frequency=d1_key.frequency,
    ...
    if len(months) != 347 or counts != {"insert": 1054, "rebuild_ohlc": 138,
                                        "replace_turnover": 26}:
        raise ValueError("BATCH_COUNTS_MOVED")
    packet = {"schema_version": "pf_w1_missing_and_turnover_prepare_v1",
...
def mds_readback(catalog, store, packet):
    ...
    if len(quality_only) != 1 or quality_only[0]["contract"] != "PF2611":
        raise ValueError("QUALITY_ONLY_CONTRACT_SCOPE_INVALID")
    through = max(date.fromisoformat(issue["week_end"]) for issue in quality_only[0]["issues"])
    endpoints = [end for end, day in mds.expected_contract_replay_endpoints(
        symbol="pf", contract="PF2611", frequency=BarFrequency.W1,
        trading_day=through, cutoff=datetime.fromisoformat(audit["as_of"]),
    ) if day == through]
    if len(endpoints) != 1:
        raise ValueError("QUALITY_ONLY_ENDPOINT_INVALID")
    targets["PF2611"] = set()
    throughs["PF2611"] = through
    cutoffs["PF2611"] = endpoints[0]
    interruptions = set()
    for contract, days in targets.items():
        bars, gaps = mds.query_contract_weekly_replay_quality(
            symbol="pf", contract=contract, through=throughs[contract], cutoff=cutoffs[contract],
    ...
    if len(expected_gaps) != 84 or interruptions != expected_gaps:
...
def main():
    ...
    try:
        ...
        if (packet["schema_version"] != "pf_w1_missing_and_turnover_prepare_v1"
                ...
                or packet["month_count"] != 347
                or packet["week_counts"] != {"insert": 1054, "rebuild_ohlc": 138,
                                              "replace_turnover": 26}
        ...
        if args.mode == "inspect":
            with Session(engine, autoflush=False) as session:
                ...
                states = []
                for record in packet["months"]:
                    key = DatasetKey("contract", "pf", record["contract"], "1w")
                    ...
                    if current == record["old_partition"]:
                        ...
                        states.append("other")
                observed_revision = catalog_revision(session, ("pf",), CUTOFF, ("1d", "1w"))
                session.rollback()
        ...
        if args.mode == "dry-run":
            ...
            print(json.dumps({"status": "dry_run_passed", "sha256": sha(raw),
                              "months": 347, "weeks": packet["week_counts"], "writes": 0}))
            return
        with Session(engine, autoflush=False) as session:
            catalog = MarketCatalog(session, root)
        ...
        print(json.dumps({"status": "committed", "sha256": sha(raw),
                          "months": 347, "weeks": packet["week_counts"], "readback": "passed"}))
    finally:
        engine.dispose()
