...
const rows = computed(() => props.items.filter((item) => !activeSector.value || item.sector === activeSector.value))
...
<template>
  <section class="market-detail" aria-labelledby="market-detail-heading">
    ...
    <div class="market-detail__scroll"><table><thead><tr><th>品种</th><th>板块</th><th>1D</th><th>5D</th><th>量比</th><th>OI变化</th><th>ATR分位</th><th>20日位置</th></tr></thead><tbody><tr v-for="item in rows" :key="item.symbol"><td><button :aria-label="`${item.symbol.toUpperCase()} ${item.product_name}`" @click="emit('open', item)">{{ item.symbol.toUpperCase() }} {{ item.product_name }}</button></td><td>{{ productSectorLabel(item.sector) }}</td><td>{{ percent(item.price_change_1d) }}</td><td>{{ percent(item.price_change_5d) }}</td><td>{{ ratio(item.volume_ratio20) }}</td><td>{{ percent(item.oi_change_1d) }}</td><td>{{ percent(item.atr14_percentile252) }}</td><td>{{ percent(item.position20) }}</td></tr></tbody></table></div>
  </section>
</template>
...
.market-detail header { display: flex; gap: 16px; align-items: end; }
