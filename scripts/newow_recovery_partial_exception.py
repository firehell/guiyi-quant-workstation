"""Pure validation for D1/W1 partial-commit authoritative source exceptions."""
...
def derive_partial_source_exceptions(
    *,
    ...
    committed = _committed_targets_from_fresh_plan(failed_unit, fresh_unit)
    if not committed:
        raise RecoveryError(ERROR_CODE)
...
def _validate_partial_source_exception(
    *,
    ...
    unit_frequency = failed_unit.get("frequency")
    if unit_frequency not in {"1d", "1w"}:
        raise RecoveryError(ERROR_CODE)
    ...
    if not _same_unit_identity(failed_unit, fresh_unit):
        raise RecoveryError(ERROR_CODE)
    if fresh_unit.get("frequency") != unit_frequency:
        raise RecoveryError(ERROR_CODE)
    committed = _committed_targets_from_fresh_plan(failed_unit, fresh_unit)
    failed_targets = _failed_targets(failed_unit, result)
...
def _replay_partial_source(
    ...
    *,
    ...
    invalid_responses: list[tuple[Mapping[str, Any], str]] = []
    for offset in range(started_count):
        ...
        if replayed:
            if replayed != {_ALLOWED_ERROR}:
                raise RecoveryError(ERROR_CODE)
            invalid_responses.append((request_payload, payload_sha256))
        expected_files.add(expected_file)
    actual_files = {path.name for path in unit_dir.glob("source-response-*.json")}
    if (
        actual_files != expected_files
        or len(invalid_responses) != 1
    ):
        raise RecoveryError(ERROR_CODE)
    failed_request_payload, failed_response_sha256 = invalid_responses[0]
    if unit.get("frequency") == "1d" and (
        date.fromisoformat(str(failed_request_payload["start"])).year,
        date.fromisoformat(str(failed_request_payload["start"])).month,
...
def _committed_targets_from_fresh_plan(
    failed_unit: Mapping[str, Any],
    fresh_unit: Mapping[str, Any],
) -> list[dict[str, Any]]:
    fresh_keys = {_target_key(item) for item in _unit_targets(fresh_unit)}
    return [
        dict(item)
        for item in _unit_targets(failed_unit)
        if _target_key(item) not in fresh_keys
    ]


def _failed_targets(
    failed_unit: Mapping[str, Any], result: Mapping[str, Any]
) -> list[dict[str, Any]]:
    failures = result.get("failures")
    ...
    return [
        dict(item)
        for item in _unit_targets(failed_unit)
        if _target_key(item) in failed_keys
    ]
