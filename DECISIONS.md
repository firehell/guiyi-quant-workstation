...
| HTDY | operational universe × 七周期 observation；持久 Alert 只记录触发窗口最新 completed Bar 的 forward-only first-seen 事实 | Scope 只认 symbol × frequency；D1/W1 只走 `canonical_updated` seam；repaint zone 历史 Bar 只供 Web 研究展示，不创建 Event 或通知；`bar_end` 是观察 Bar 时间，`detected_at` 是首次识别时间；同一 Event identity 后续重绘一律 immutable no-op |
