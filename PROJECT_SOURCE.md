...
长期允许按人工 Gate 分阶段评估 `研究观察 → Paper → Shadow → Broker Read-only → 订单草稿 → 人工确认 → 半自动 → 受控自动`；这不是当前能力或晋升授权。AI 可自动研究，不能自动晋升、修改 active 策略或决定真实账户持仓。苏冰课程 Knowledge Foundation 属于独立 AutoTrade 项目，不在本仓库维护课程开发计划、代码、私有数据或 Runtime 依赖。
...
- Market 详情的已接受产品合同使用同一稳定工作台中的六个扁平入口：`震荡策略 / 趋势策略 / 主升浪 / HTDY / SuBing / Free`；前三者仍是三个独立 Newow 身份。Newow 允许显示策略 `BUILD/HOLD/CLEAR/FLAT` 状态、主动作、Hint、ReferenceTrade 和明确标注的乐观参考摘要；旧 `view=trend` 页面链接一次性迁移到 `view=newow&strategy=trend&series_kind=actual_dominant&frequency=1d`，`/api/v1/market/newow/trend-detail` 仍仅保留固定 D1 API 兼容语义。其他视角不得消费或复制这些 Newow 事实。
...
- Newow 详情采用局部白色全宽 Shell、单层策略入口、唯一周期入口、原位展开解释与原生弹窗；K 线、成交量和单一副图共享时间轴。同品种同周期切换 Newow 策略时只替换策略所属图层，并仅在时间轴完全兼容时保留缩放；身份或时间轴不兼容时清空旧图层并重置。默认 MACD 为既有内核的只读显示（12/26/9、sma_window、histogram×2），不声明牛哇 MACD 原站 parity；其他副图替换同一 pane。
...
允许先交付趋势、震荡、主升浪 × `1d/1w` 六组合，再独立交付三个 `60m` 组合；
每次须声明品种、历史窗口和面板支持范围。阶段目标、实际发布/开放和未完成 Gate 只看 `STATUS.md`。
本节记录已接受的分阶段边界，不声明日周限制已在现有代码或 Runtime 生效。

日周版只收窄 Newow 本版开放面，不删除数据中心分钟链路或通用 60m 能力，
不改变 HTDY/SuBing/Free 合同与持续授权。同周期主图、主动作/状态、选定副图和 ReferenceTrade
按各自输入独立验收；依赖未开放周期的完整综合解释暂缓开放并明确原因，不能拖垮已验证面板。
不得仅隐藏按钮而让旧链接、保存偏好或 API 继续隐式进入未开放能力；实施前须在任务合同及
相关 OpenSpec 中明确入口与状态处理，不能仅靠本次文档修改声称完成。

缺少 60m 输入不能通过沿用原综合总分、补默认值或创造日周简化评分解决。跨周期能力恢复时仍须
按各周期 completed Bar、`bar_end` 与 `as_of` 验证；数据已下载/派生不等于产品已开放。
