#!/usr/bin/env bash
# 通达信 XMA 通道策略 — RQAlpha Plus 回测（默认 --plot）
set -euo pipefail
...
if [[ ! -d "$VENV" ]]; then
  echo "未找到 RQAlpha 虚拟环境。请先按 experiments/rqalpha_jm_buy_hold/README.md 初始化 .venv"
  exit 1
fi
...
export TDX_XMA_STRATEGY_DIR="$ROOT"
...
"$VENV/bin/rqalpha-plus" run \
  -f "$ROOT/tdx_xma_bands_strategy.py" \
  -s "$START_DATE" \
  -e "$END_DATE" \
  -fq 1d \
