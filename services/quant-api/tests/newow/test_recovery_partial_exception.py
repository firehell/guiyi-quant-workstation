...
def _write_weekly_unit_evidence(
    parent: Path,
) -> tuple[Path, dict[str, Any], dict[str, Any], list[dict[str, Any]]]:
    parent.mkdir(parents=True, exist_ok=True)
    unit_dir = parent / "unit-001-eb-EB2403"
    unit_dir.mkdir()
    daily = [_target(month, symbol="eb", contract="EB2403") for month in range(3, 8)]
    weekly = [
        {
            **_target(month, symbol="eb", contract="EB2403"),
            "dataset": ["contract", "eb", "EB2403", "1w"],
    ...
    sources = [
        ExchangeDailySourceRequest(
            contract="EB2403",
            start=date(2023, month, 1),
            end=date(2023, month, 2),
            expected_dates=(date(2023, month, 1), date(2023, month, 2)),
        )
        for month in range(3, 8)
    ]
    journal = AttemptJournal(unit_dir, tuple(sources))
    for request in sources:
        journal.before_request(request)
        journal.after_response(
            request,
            tuple(
                _row(day, invalid=request.start.month == 7)
    ...
    committed = daily[:-1] + weekly[:-1]
    return unit_dir, failed_unit, fresh_unit, committed
...
def test_weekly_partial_source_exception_uses_fresh_plan_difference(
    tmp_path: Path,
) -> None:
    native_dir = tmp_path / "batch-001" / "native"
    unit_dir, _failed, fresh_unit, committed = _write_weekly_unit_evidence(native_dir)
    ...
    assert payload["frequency"] == "1w"
    assert payload["committed_targets"] == committed
    assert {item["dataset"][3] for item in payload["committed_targets"]} == {
        "1d",
        "1w",
    }
    assert payload["remaining_targets"] == fresh_unit["targets"]
    assert payload["source_request_identity"]["start"] == "2023-07-01"
...
def test_partial_source_exception_fail_closed(
    ...
    elif mutation == "unsupported_frequency":
        result = json.loads((kwargs["unit_dir"] / "unit-result.json").read_text())
        result["frequency"] = "60m"
        (kwargs["unit_dir"] / "unit-result.json").write_text(json.dumps(result))
        kwargs["fresh_unit"] = {**kwargs["fresh_unit"], "frequency": "60m"}
