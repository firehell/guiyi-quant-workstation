...
| 目标/吸筹显示选择 | 三策略共享 `1w/1d/60m` context | `newow_target_absorb_display_selection_page_v2` | 无 active Core；原件为 `M-SOURCE`、冻结派生为 `M-CORE` | BASE 无产品测试 | `sources/strategy-calc-v3.2.82.js`、`analysis/core-parity-inputs.json`、`analysis/core-page-parity-results.json` | `RESEARCH_EVIDENCE_ONLY` | P3 Task 11 必须核对完整日/周选择、周线覆盖、昨收、clamp、warm-up 后再实现；当前不能宣称产品完成 |
