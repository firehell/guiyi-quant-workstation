"""Exact PR W1 missing-week and receipt-backed turnover repair."""
...
PREPARED = HERE / "pr-w1-prepare.json"
...
def load_targets():
    ...
    if (audit["products"] != ["pr"] or audit["revision_stable"] is not True
            or audit["provider_requests"] != 0 or audit["writes"] != 0
            or len(audit["contracts"]) != 16
    ...
    if (dict(counts) != {"W1_MISSING_D1_COMPLETE": 389,
                         "D1_W1_VALUE_CONFLICT": 12,
                         "STRICT_NO_TRADE_AGGREGATION": 19,
                         "PROVEN_QUALITY_INTERRUPTION": 106}
            or len(targets) != 14):
        raise ValueError("ISSUE_COUNTS_MOVED")
    receipts = json.loads(RECEIPTS.read_bytes())
    evidence = {}
    for row in receipts["rows"]:
        if row["symbol"] != "pr" or not row["all_daily_receipted"]:
            continue
            ...
            for receipt_path in part["matching_receipts"]:
                ...
                if (receipt["status"] != "passed" or receipt["contract"] != row["contract"]
                        ...
                        or not any(item["year"] == part["year"] and item["month"] == part["month"]
                                   and item["dataset"] == ["contract", "pr", row["contract"], "1d"]
...
def build_packet(session, root: Path, config_identity):
    ...
    revision = catalog_revision(session, ("pr",), CUTOFF, ("1d", "1w"))
    ...
    for contract in sorted(targets):
        d1_key = DatasetKey("contract", "pr", contract, "1d")
        w1_key = DatasetKey("contract", "pr", contract, "1w")
        ...
        for issue in targets[contract]:
            ...
            weekly_points = [point for point in mds.expected_contract_replay_endpoints(
                symbol="pr", contract=contract, frequency=w1_key.frequency,
            ...
            expected = tuple(point for point in mds.expected_contract_replay_endpoints(
                symbol="pr", contract=contract, frequency=d1_key.frequency,
    ...
    if len(months) != 122 or counts != {"insert": 389, "rebuild_ohlc": 19,
                                        "replace_turnover": 12}:
        raise ValueError("BATCH_COUNTS_MOVED")
    packet = {"schema_version": "pr_w1_missing_and_turnover_prepare_v1",
...
def mds_readback(catalog, store, packet):
    ...
    for contract, days in targets.items():
        bars, gaps = mds.query_contract_weekly_replay_quality(
            symbol="pr", contract=contract, through=throughs[contract], cutoff=cutoffs[contract],
    ...
    if len(expected_gaps) != 106 or interruptions != expected_gaps:
...
def main():
    ...
    try:
        ...
        if (packet["schema_version"] != "pr_w1_missing_and_turnover_prepare_v1"
                ...
                or packet["month_count"] != 122
                or packet["week_counts"] != {"insert": 389, "rebuild_ohlc": 19,
                                              "replace_turnover": 12}
        ...
        if args.mode == "inspect":
            with Session(engine, autoflush=False) as session:
                ...
                states = []
                for record in packet["months"]:
                    key = DatasetKey("contract", "pr", record["contract"], "1w")
                    ...
                    if current == record["old_partition"]:
                        ...
                        states.append("other")
                observed_revision = catalog_revision(session, ("pr",), CUTOFF, ("1d", "1w"))
                session.rollback()
