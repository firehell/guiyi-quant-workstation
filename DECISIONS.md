...
| Alert develop 通知目标 | `AlertEvent commit → WeChatGroupAlertSender → pinned WeChat-Courier child → 唯一固定群最多一次 GUI send`；exact clean commit、exact module shape、唯一精确 search result、精确 chat title 与非阻塞 GUI lock 全部 fail-closed | 不引入 OpenClaw/iLink/inbound/context monitor/Agent/LLM；不使用 upstream fuzzy match 作为安全边界；无 retry/queue/replay/backfill/outbox/failover；production exact-tag 仍为 WeCom，需未来独立 rollout/release/promotion |
