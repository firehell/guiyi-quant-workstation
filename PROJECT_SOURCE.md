...
本节记录已接受的分阶段边界；实际数据与 Runtime 验收仍以 `STATUS.md` 和现场证据为准。

日线版只收窄 Newow 本版开放面，不删除数据中心分钟链路或通用 60m 能力，
