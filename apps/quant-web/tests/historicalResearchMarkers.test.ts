...
test('SuBing Strategy marker anchors effective Bar and shows all close reasons', () => {
  ...
  assert.match(open.tooltip!, /决策 2026-08-03T01:05:00Z/)
  assert.match(open.tooltip!, /参考价 100/)
  assert.doesNotMatch(open.tooltip!, /持有/)
  assert.doesNotMatch(open.tooltip!, /参考变动/)
  assert.doesNotMatch(open.tooltip!, /107\.97/)
  assert.equal(close.label, '× 清多')
  assert.equal(close.position, 'aboveBar')
  assert.match(close.tooltip!, /入场决策 2026-08-03T01:05:00Z/)
  assert.match(close.tooltip!, /入场生效 2026-08-03T01:10:00Z/)
  assert.match(close.tooltip!, /入场参考价 100/)
  assert.match(close.tooltip!, /平仓决策 2026-08-03T01:10:00Z/)
  assert.match(close.tooltip!, /平仓生效 2026-08-03T01:15:00Z/)
  assert.match(close.tooltip!, /平仓参考价 107\.97/)
