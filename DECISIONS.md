...
| release | `release-flow.sh prepare/publish/tag` 以精确 current/target/rollback SHA 受控完成本地 fast-forward、远端 refs 与 annotated tags | 三动作默认 dry-run；publish 绑定批准时的远端 main 且不改 upstream；tag 仅对 object/target/message 全匹配的相同 packet 幂等；用户批准 exact packet 后才可逐项 apply；task→develop 自动集成不授权 release、tag 或 Runtime |
