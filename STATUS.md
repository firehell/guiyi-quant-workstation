...
| Runtime | v1.9.10 已创建 clean、detached 的 `/Volumes/扩展盘/guiyi-quant-runtime-v1.9.10-r1@a56d13cd`。首次未预置 API `.venv` 与 Web runtime assets 的启动失败后已回滚；取得新的明确授权后，已在该 root 预置锁定 assets 并完成一次五项服务切换。API/Web/Live/Alert 均 running，盘后服务按调度 `not_running`，五项 exact root/commit 一致。当前为 CLOSED：Live `60` 个产品无订阅且 `last_bar_at=null`；Alert 正在 startup restore（`warming / alert_heartbeat_missing`），因此 Runtime 仍为 degraded，不能标记 `RUNTIME_READY`。未执行盘后、未改 Scope/Rule、未手工发送通知。 |
