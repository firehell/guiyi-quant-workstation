"""Exact SF W1 missing-week and receipt-backed turnover repair."""
...
PREPARED = HERE / "sf-w1-prepare.json"
...
def load_targets():
    ...
    if (audit["products"] != ["sf"] or audit["revision_stable"] is not True
            or audit["provider_requests"] != 0 or audit["writes"] != 0
            or len(audit["contracts"]) != 28
    ...
    if (dict(counts) != {"W1_MISSING_D1_COMPLETE": 725,
                         "D1_W1_VALUE_CONFLICT": 19,
                         "STRICT_NO_TRADE_AGGREGATION": 33,
                         "PROVEN_QUALITY_INTERRUPTION": 10}
            or len(targets) != 23):
        raise ValueError("ISSUE_COUNTS_MOVED")
    receipts = json.loads(RECEIPTS.read_bytes())
    evidence = {}
    for row in receipts["rows"]:
        if row["symbol"] != "sf" or not row["all_daily_receipted"]:
            continue
            ...
            for receipt_path in part["matching_receipts"]:
                ...
                if (receipt["status"] != "passed" or receipt["contract"] != row["contract"]
                        ...
                        or not any(item["year"] == part["year"] and item["month"] == part["month"]
                                   and item["dataset"] == ["contract", "sf", row["contract"], "1d"]
...
def build_packet(session, root: Path, config_identity):
    ...
    revision = catalog_revision(session, ("sf",), CUTOFF, ("1d", "1w"))
    ...
    for contract in sorted(targets):
        d1_key = DatasetKey("contract", "sf", contract, "1d")
        w1_key = DatasetKey("contract", "sf", contract, "1w")
        ...
        for issue in targets[contract]:
            ...
            weekly_points = [point for point in mds.expected_contract_replay_endpoints(
                symbol="sf", contract=contract, frequency=w1_key.frequency,
            ...
            expected = tuple(point for point in mds.expected_contract_replay_endpoints(
                symbol="sf", contract=contract, frequency=d1_key.frequency,
            ...
            if issue["kind"] == "D1_W1_VALUE_CONFLICT":
                allowed_fields = (["volume", "turnover"] if
                                  (contract, day.isoformat()) == ("SF2506", "2025-04-11")
                                  else ["turnover"])
                if issue["fields"] != allowed_fields:
        ...
        for (year, month), items in sorted(grouped.items()):
            ...
            for issue, new_bar, week_sources, receipts, endpoints in items:
                ...
                if issue["kind"] == "W1_MISSING_D1_COMPLETE":
                    ...
                    action = ("replace_volume_turnover" if len(diff) == 2
                              else "replace_turnover")
    ...
    counts = {name: sum(change["action"] == name for row in months for change in row["changes"])
              for name in ("insert", "rebuild_ohlc", "replace_turnover",
                           "replace_volume_turnover")}
    if len(months) != 211 or counts != {"insert": 725, "rebuild_ohlc": 33,
                  "replace_turnover": 18, "replace_volume_turnover": 1}:
        raise ValueError("BATCH_COUNTS_MOVED")
    packet = {"schema_version": "sf_w1_targeted_repair_prepare_v1",
...
def mds_readback(catalog, store, packet):
    ...
    for contract, days in targets.items():
        bars, gaps = mds.query_contract_weekly_replay_quality(
            symbol="sf", contract=contract, through=throughs[contract], cutoff=cutoffs[contract],
    ...
    if len(expected_gaps) != 10 or interruptions != expected_gaps:
...
def main():
    ...
    try:
        ...
        if (packet["schema_version"] != "sf_w1_targeted_repair_prepare_v1"
                ...
                or packet["month_count"] != 211
                or packet["week_counts"] != {"insert": 725, "rebuild_ohlc": 33,
                                              "replace_turnover": 18,
                                              "replace_volume_turnover": 1}
        ...
        if args.mode == "inspect":
            with Session(engine, autoflush=False) as session:
                ...
                states = []
                for record in packet["months"]:
                    key = DatasetKey("contract", "sf", record["contract"], "1w")
                    ...
                    if current == record["old_partition"]:
                        ...
                        states.append("other")
                observed_revision = catalog_revision(session, ("sf",), CUTOFF, ("1d", "1w"))
                session.rollback()
