...
const props = defineProps<{ targetAsOf: string | null; asOf: string | null; participants: number | null; active: number | null; staleCount: number | null; unavailableCount: number | null; priceUnavailableCount: number | null; overview: string; runtime: string | null; overviewStale: boolean; runtimeStale: boolean; overviewError: 'request-failed' | 'typed-unavailable' | null; afterMarket?: RuntimeAfterMarketHealth | null; weeklyAudit?: RuntimeWeeklyAuditHealth | null }>()
...
<template>
  <section class="trust" role="status">
    ...
    <details>
      ...
      <div>目标日期 {{ targetAsOf ?? '—' }} · 数据日期 {{ asOf ?? '—' }}<br>overview {{ overview }} · Runtime {{ runtime ?? 'unavailable' }}<span v-if="overviewError"><br>overview {{ overviewError }}</span></div>
    </details>
  </section>
</template>
