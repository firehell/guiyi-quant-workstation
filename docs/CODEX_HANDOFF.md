...
- 8.5-5 锁定 `jm.MAIN` 与真实 `actual_contract` historical bars 分离，后续 8.5-6 写入必须独立文件、独立质量报告、独立 active Gate。
...
明确授权后做 JM-only 当前真实主力合约 historical bars 最小写入试点；未授权前不得写真实行情数据，不得登记 active，不得接企业微信。
