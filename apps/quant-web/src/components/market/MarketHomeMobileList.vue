...
import { marketHomeQualityNotice } from '@/utils/marketHomePresentation'
...
<template><section class="mobile-list" aria-label="移动端品种列表"><button v-for="row in rows" :key="row.symbol" @click="$emit('open',row)"><strong>{{row.symbol.toUpperCase()}} {{row.product_name}}</strong><span>{{ row.liveQuote ? `${row.liveQuote.physicalContract} · ${liveStale ? '断线保留价' : row.liveQuote.source === 'completed_1m' ? '最新 1m' : '历史收盘'}` : '日线收盘' }} {{row.close}} · {{ row.liveQuote ? '同合约较昨收' : '1d 涨跌幅' }} {{percentage(row.price_change_1d)}}</span><small v-if="marketHomeQualityNotice(row.reason_codes)">{{ marketHomeQualityNotice(row.reason_codes) }}</small><i><b>日 <MarketStateIcon :state="row.dailyState"/></b><b>周 <MarketStateIcon :state="row.weeklyState"/></b><b>同向 <MarketStateIcon :state="alignment(row.alignment)"/></b></i></button></section></template>
