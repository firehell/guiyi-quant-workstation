...
main() {
  ...
  "${PROJECT_ROOT}/scripts/dev/dev-healthcheck.sh"

  info "本脚本未重启 FRPC；如隧道异常，由人工执行: brew services restart frpc"
  info "Mac mini 隧道诊断: ${PROJECT_ROOT}/scripts/ops/network/local-tunnel-healthcheck.sh"
  info "腾讯云隧道验收: ${PROJECT_ROOT}/scripts/ops/network/tunnel-healthcheck.sh"
