...
function instant(value: unknown, field: string): string { if (typeof value !== 'string' || !/T.*(?:Z|[+-]\d{2}:?\d{2})$/.test(value) || !Number.isFinite(Date.parse(value))) throw new Error(`${field} must be an ISO instant with timezone`); return value }
