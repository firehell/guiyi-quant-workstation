"""Read-only formal RS W1 API smoke on development code and actual Catalog."""
...
try:
    with TestClient(app) as client:
        ...
        if (cap.status_code != 200
                or capability["schema_version"] != "newow_product_capabilities_v14"
                or len(capability["weekly_products"]) != 52
                or "rs" not in capability["weekly_products"]
        ...
        expected = {
            ...
            "oscillation": {"chart": "warming", "reference": "warming", "auxiliary": "warming", "comparator": "unavailable"},
        ...
        for strategy, statuses in expected.items():
            for section, expected_status in statuses.items():
                params = {
                    "product": "rs", "strategy": strategy, "frequency": "1w", "section": section,
        ...
        result = {"status": "passed", "scope_count": 52, "schema_version": capability["schema_version"],
                  "sections": sections, "pf_still_closed": True, "database_mode": "read_only"}
        (Path(__file__).parent / "rs-formal-scope-smoke.json").write_text(
