...
| Alert develop 通知目标 | `AlertEvent commit → ClawbotAlertSender → one Node child → exact openclaw-weixin private seam → sendMessageWeixin()`；owner 配置不可变，exact OpenClaw/Node/plugin version、root 与 compiled module shape fail-closed；每个 Event 最多一个 child、一次发送 primitive | OpenClaw 是既有外部依赖，不由归一量化安装、更新、登录或监督；不使用 public message-send、durable queue、inbound/Agent/LLM；无 retry/queue/replay/backfill/outbox/failover；production exact-tag 仍为 WeCom，需未来独立 rollout/release/promotion |
