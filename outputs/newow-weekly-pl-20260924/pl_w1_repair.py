"""Exact PL W1 missing-week and receipt-backed turnover repair."""
...
PREPARED = HERE / "pl-w1-prepare.json"
...
def load_targets():
    ...
    if (audit["products"] != ["pl"] or audit["revision_stable"] is not True
            or audit["provider_requests"] != 0 or audit["writes"] != 0
            or len(audit["contracts"]) != 7
    ...
    if (dict(counts) != {"W1_MISSING_D1_COMPLETE": 63,
                         "D1_W1_VALUE_CONFLICT": 5,
                         "PROVEN_QUALITY_INTERRUPTION": 89}
            or len(targets) != 5):
        raise ValueError("ISSUE_COUNTS_MOVED")
    receipts = json.loads(RECEIPTS.read_bytes())
    evidence = {}
    for row in receipts["rows"]:
        if row["symbol"] != "pl" or not row["all_daily_receipted"]:
            continue
            ...
            for receipt_path in part["matching_receipts"]:
                ...
                if (receipt["status"] != "passed" or receipt["contract"] != row["contract"]
                        ...
                        or not any(item["year"] == part["year"] and item["month"] == part["month"]
                                   and item["dataset"] == ["contract", "pl", row["contract"], "1d"]
...
def build_packet(session, root: Path, config_identity):
    ...
    revision = catalog_revision(session, ("pl",), CUTOFF, ("1d", "1w"))
    ...
    for contract in sorted(targets):
        d1_key = DatasetKey("contract", "pl", contract, "1d")
        w1_key = DatasetKey("contract", "pl", contract, "1w")
        ...
        for issue in targets[contract]:
            ...
            weekly_points = [point for point in mds.expected_contract_replay_endpoints(
                symbol="pl", contract=contract, frequency=w1_key.frequency,
            ...
            expected = tuple(point for point in mds.expected_contract_replay_endpoints(
                symbol="pl", contract=contract, frequency=d1_key.frequency,
    ...
    if len(months) != 24 or counts != {"insert": 63, "rebuild_ohlc": 0,
                                        "replace_turnover": 5}:
        raise ValueError("BATCH_COUNTS_MOVED")
    packet = {"schema_version": "pl_w1_missing_and_turnover_prepare_v1",
...
def mds_readback(catalog, store, packet):
    ...
    for contract, days in targets.items():
        bars, gaps = mds.query_contract_weekly_replay_quality(
            symbol="pl", contract=contract, through=throughs[contract], cutoff=cutoffs[contract],
    ...
    if len(expected_gaps) != 89 or interruptions != expected_gaps:
...
def main():
    ...
    try:
        ...
        if (packet["schema_version"] != "pl_w1_missing_and_turnover_prepare_v1"
                ...
                or packet["month_count"] != 24
                or packet["week_counts"] != {"insert": 63, "rebuild_ohlc": 0,
                                              "replace_turnover": 5}
        ...
        if args.mode == "inspect":
            with Session(engine, autoflush=False) as session:
                ...
                states = []
                for record in packet["months"]:
                    key = DatasetKey("contract", "pl", record["contract"], "1w")
                    ...
                    if current == record["old_partition"]:
                        ...
                        states.append("other")
                observed_revision = catalog_revision(session, ("pl",), CUTOFF, ("1d", "1w"))
                session.rollback()
