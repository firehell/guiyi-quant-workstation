...
- PR #225 首次 exact-head 独立 Review 发现：既有 Lifecycle 的多头突破高点/空头突破低点被错误当成批准设计所需的多头绑定低点/空头绑定高点，另有平仓 Marker 位置和 Tooltip 事实缺口。代码已按 RED→GREEN 修正：非定向结构 Pivot 不再被重标，只记为 `structure_exit_available=false`，建仓仍保留；Marker 位置与完整事实已修正。修正后四品种 `structure_exit_available=0`、合法 bound-low/bound-high 自然退出数为 0，说明当前权威 Lifecycle 事实没有提供批准 Spec 所需的结构 Pivot；不得用突破 Pivot 冒充或隐式搜索替代。
