...
test('loads the server-owned daily release capability and rejects widened or legacy payloads', async () => {
  ...
  const formalWeeklyV11 = {
    ...formalWeeklyV10,
    schema_version: 'newow_product_capabilities_v11',
    weekly_products: 'a ag al ao ap au b bu bz c cf cu eb ec eg fg fu hc i j jd jm l lc lh m ma ni oi p pb pd pg pp ps pt rb rm ru sa sc si sn ss ta ur v y zn'.split(' '),
  }
  assert.deepEqual(await getNewowProductCapabilities({
    request: async () => formalWeeklyV11,
  }), formalWeeklyV11)
  const formalWeeklyV12 = {
    ...formalWeeklyV11,
    schema_version: 'newow_product_capabilities_v12',
    weekly_products: 'a ag al ao ap au b bu bz c cf cj cu eb ec eg fg fu hc i j jd jm l lc lh m ma ni oi p pb pd pg pp ps pt rb rm ru sa sc si sn ss ta ur v y zn'.split(' '),
  }
  assert.deepEqual(await getNewowProductCapabilities({
  ...
  for (const capability of [candidate, candidateV9, formalWeeklyV10, formalWeeklyV11, formalWeeklyV12, formalWeeklyV13, formalWeeklyV14, formalWeeklyV15, formalWeeklyV16, formalWeeklyV17, formalWeeklyV18, formalWeeklyV19, formalWeeklyV20, formalWeeklyV21]) {
  ...
  assert.deepEqual(await getNewowProductCapabilities({ request: async () => pdPtHourly }), pdPtHourly)
  await assert.rejects(
    getNewowProductCapabilities({ request: async () => ({ ...pdPtHourly, release_stage: 'ap_hourly_candidate' }) }),
    (error: unknown) => error instanceof NewowProductRequestError && error.code === 'NEWOW_RESPONSE_INVALID',
  )
