...
| Runtime | `2026-09-08 09:41 CST` 只读核对为五服务混合版本：API/Web 已加载 clean、detached `/Volumes/扩展盘/guiyi-quant-runtime-v1.10.0-r1@f8f7d91765122c33cf5e82ed425b6c44f41ad0b1`；Live/After-market/Alert 仍加载 `/Volumes/扩展盘/guiyi-quant-runtime-v1.9.15-r1@36fef03923a168145e6fd2eab023dc1d2b411ad6`。v1.10.0根下Market/Alert marker均缺失，API/Web为200、Runtime health为`ok / readonly=true`，但`local-services-status.sh` 为 `overall=failed, failures=9`。该状态不是v1.10.0五服务promotion，也不是`RUNTIME_READY`；本轮未切换、重启或回滚任何服务。v1.9.15曾完成的`RUNTIME_READY / SUBING_NATURAL_CLOSURE_COMPLETE`保留为历史发布事实，不能代替当前同版身份验收。 |
| Runtime root 核对 | `guiyi-quant-runtime-v1.10.0-r1@f8f7d917...`与`guiyi-quant-runtime-v1.9.15-r1@36fef039...`均为clean、detached；前者是当前supervised root，后者仍被三项已加载服务引用，两者均不得删除。旧`guiyi-quant-runtime-v1.9.14-r1`仍不存在。 |
...
| 当前 mixed health | `2026-09-08T09:45:07 Asia/Shanghai` 已加载v1.10.0 API返回`status=ok / readonly=true`且三类would-side-effect均为false；由于v1.10.0根下marker未启用，Live/After-market/Alert在该health中均为`configured_enabled=false`，Alert为`unobserved`。Live仍可从共享状态读到60品种订阅和心跳，但另一个v1.9.15 launchd进程正在运行；这是跨版本观测，不得解读为当前Alert/Live同版健康证据。 |
| 最近自然 After-market | 2026-09-07 的自然 18:05 任务于 `19:30:30 Asia/Shanghai` 完成，`status=passed`、`attempts=1`、`last_successful_trading_day=2026-09-07`、`last_failure=null`；任务已退出。这是混合切换前的v1.9.15历史事实；当前v1.10.0 API health因marker缺失将After-market视为`disabled`。 |
...
- 苏冰截至 `2026-09-04` 的历史物理 15m 已 `60/60 VERIFIED`，本轮 50 个合约全部成功，无 partial、failed、blocked 或未开始项；该历史目标已完成。BU/BZ 旧七周期 warm-up 的 D1/W1 问题仍未关闭，尤其 BZ `2026-03-20` 非零成交零 O/H/L 不得用 close、settlement 或 1m 代替；它不再阻塞限定 `1m → 15m` 的苏冰历史输入验收。最后一次同版v1.9.15 Rule health曾自然恢复为`error_type=null`，但当前是API/Web v1.10.0与Live/Alert v1.9.15的混合状态，且本轮没有取得一份有效的60品种当前readiness汇总，因此不声称当前Live输入逐品种60/60 ready。恢复候选的后续版本发布、exact-tag五服务promotion与生产Live recovery enable仍是独立Gate；启用前必须核对Live/Alert为同一exact root/version且恢复协议同时启用。盘后`missed`已由2026-09-07自然passed关闭。
...
- HTDY目标61对Scope未应用；任何Scope调整、真实数据修复、通知、main/tag/release或Runtime版本切换仍需目标/环境/范围明确的单次执行意图。本轮仓库修复不改变当前API/Web v1.10.0与Live/After-market/Alert v1.9.15的混合运行状态。
