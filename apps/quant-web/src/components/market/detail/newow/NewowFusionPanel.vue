...
<template>
  <section class="fusion-panel" aria-label="双策略融合参考模型">
    <header><div><strong>双策略融合参考</strong><p>趋势 / 震荡 / 融合独立统计 · 单仓 long/flat · 零成本页面参考 · 不代表可执行收益</p></div><button :disabled="loading" @click="load">{{ loading ? '计算中…' : result ? '重新计算' : '查看三组结果' }}</button></header>
    ...
    <template v-if="result">
      ...
      <div class="fusion-panel__scroll"><table><thead><tr><th>模型</th><th>已完成</th><th>累计收益百分点</th><th>未清仓</th><th>中断</th></tr></thead><tbody><tr v-for="g in result.groups" :key="g.model"><th>{{ names[g.model] }}</th><td>{{ g.closed_count }}</td><td :class="tone(g.sum_return_percentage_points)">{{ display(g.sum_return_percentage_points) }}</td><td>{{ g.open_count }}</td><td>{{ g.interrupted_count }}</td></tr></tbody></table></div>
      <p>累计只简单相加窗口内建仓且已完成的参考收益；未清仓浮动、中断、期初已有单独展示。</p>
      <details open><summary>融合交易记录（{{ result.items.length }} 条{{ result.records_truncated ? '，仅显示最近 200 条，统计为完整窗口' : '' }}）</summary><div class="fusion-panel__scroll"><table><thead><tr><th>合约 / 状态</th><th>建仓来源 / 时间 / 价格</th><th>清仓来源 / 时间 / 价格</th><th>已完成收益</th><th>未清仓浮动 / 中断前变化</th></tr></thead><tbody><tr v-for="r in result.items" :key="r.reference_trade_id"><td>{{ r.physical_contract }} · {{ states[r.status] }}<small v-if="r.statistics_membership === 'initial_before_window'">期初已有，不计入统计</small></td><td>{{ names[r.entry_source] }}<small>{{ formatBeijingInstant(r.entry_bar_end) }}</small>{{ display(r.entry_reference_price) }}</td><td>{{ r.exit_source ? names[r.exit_source] : '—' }}<small>{{ r.exit_bar_end ? formatBeijingInstant(r.exit_bar_end) : '—' }}</small>{{ display(r.exit_reference_price) }}</td><td :class="tone(r.reference_return_pct)">{{ display(r.reference_return_pct) }}{{ r.reference_return_pct !== null ? '%' : '' }}</td><td>{{ r.status === 'CLOSED' ? '—' : display(r.mark_change_pct) + (r.mark_change_pct !== null ? '%' : '') }}</td></tr></tbody></table></div></details>
    </template>
  </section>
</template>
<style scoped>
