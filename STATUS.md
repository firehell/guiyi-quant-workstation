...
- SuBing 增量快照在 feature `subing-performance-incremental-snapshots` 达到 `CODE_COMPLETE` / `TEST_COMPLETE`：HTTP 只读校验当前 schema-v3 快照与 current manifest；盘后 derived 阶段按物理段尾决策 `UNCHANGED` / `REPLAY_FROM_SEGMENT` / `FULL_REBUILD_REQUIRED` 并原子发布完整当前快照；CLI `--warm-cache` 仍是明确全历史 bootstrap。2026-08-28 仓库定向 pytest 209 passed，ruff All checks passed，mypy Success 127 source files，canonical consistency 8 passed，secret_scan `finding_count=0`，`git diff --check` 无空白错误。该证据不构成真实 schema-v2 采纳、active60 刷新、Runtime promotion，也不声明 `RELEASED` 或 `RUNTIME_READY`。
