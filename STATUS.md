...
| Database | 2026-09-03 只读 readback：production Alembic 为 `20260903_0045`；184,405 条 RQData Session 已规范化且旧锚点为 0；Rule 恰为 enabled HTDY `jm × 15m` 与 disabled、empty-scope `subing_ths_alert_15m_v1`；SuBing Event 为 0。 |
| Market Runtime Scope | `operational_products.txt` 的 60 个品种。 |
| Alert Scope | HTDY 为 `jm × 15m`；SuBing 严格保持 `disabled + empty scope`。修正后 G10 已通过。2026-09-03 G9 dry-run 为 `planned / readonly=true`：execution-time operational universe 为 60 个品种，Scope hash `ce1daca77aeb1abe134806b67aebd96b2c35db3ba82aa10af58f6e5a2e4f5fa2`；尚未执行 `--apply`。 |
...
- G9 production Scope activation + Rule enable 尚未执行。当前已核验的 dry-run 为 60 个品种、Scope hash `ce1daca77aeb1abe134806b67aebd96b2c35db3ba82aa10af58f6e5a2e4f5fa2`；真实 `subing-ths-scope --apply` 必须在执行时重新 preflight，并取得一次新的单次明确授权。
