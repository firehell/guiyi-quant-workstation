"""Read-only formal SF W1 API smoke on development code and actual Catalog."""
...
try:
    with TestClient(app) as client:
        ...
        if (cap.status_code != 200
                or capability["schema_version"] != "newow_product_capabilities_v20"
                or len(capability["weekly_products"]) != 58
                or "sf" not in capability["weekly_products"]
                or any(p in capability["weekly_products"] for p in (
                    "sh", "sm"
                ))):
            raise ValueError("CAPABILITY_SCOPE_INVALID")
        expected = {
            "trend": {"chart": "ready", "reference": "ready", "auxiliary": "ready", "comparator": "not_applicable"},
            "oscillation": {"chart": "ready", "reference": "ready", "auxiliary": "ready", "comparator": "unavailable"},
            "main_rise": {"chart": "ready", "reference": "ready", "auxiliary": "ready", "comparator": "not_applicable"},
        }
        sections = []
        for strategy, statuses in expected.items():
            for section, expected_status in statuses.items():
                params = {
                    "product": "sf", "strategy": strategy, "frequency": "1w", "section": section,
        ...
        closed = client.get(
            ...
            params={"product": "sh", "strategy": "trend", "frequency": "1w", "section": "chart"},
        )
        ...
        result = {"status": "passed", "scope_count": 58, "schema_version": capability["schema_version"],
                  "sections": sections, "sh_still_closed": True, "database_mode": "read_only"}
        (Path(__file__).parent / "sf-formal-scope-smoke.json").write_text(
