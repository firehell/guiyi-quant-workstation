...
class SuBingJmDailyEma21MacdVolumeParams:
    ...
    macd_slow: int = 26
    macd_signal: int = 9
    jm_macd_zero_band: float = 25
    volume_confirm_enabled: bool = True
    volume_rule: str = "current_volume_gt_previous_volume"
...
def validate_params(raw_params: Mapping[str, Any] | None = None) -> SuBingJmDailyEma21MacdVolumeParams:
    allowed = {field.name for field in fields(SuBingJmDailyEma21MacdVolumeParams)}
    params = dict(DEFAULT_PARAMS)
    params.update({key: value for key, value in dict(raw_params or {}).items() if key in allowed})
    validated = SuBingJmDailyEma21MacdVolumeParams(**params)
    ...
    _validate_exact_number("jm_macd_zero_band", validated.jm_macd_zero_band, 25)
    if validated.macd_fast >= validated.macd_slow:
        raise ValueError("macd_fast must be less than macd_slow")
    if validated.volume_confirm_enabled is not True:
        raise ValueError("volume_confirm_enabled must stay true")
    if validated.volume_rule != "current_volume_gt_previous_volume":
        raise ValueError("volume_rule must be current_volume_gt_previous_volume")
...
def _validate_exact_number(name: str, value: float, expected: float) -> None:
