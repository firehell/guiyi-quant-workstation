...
def test_formal_sr_weekly_scope_uses_v2_and_keeps_other_seven_closed():
    ...
    for product in ("pf", "pl", "pr", "px", "sf", "sh", "sm"):
        with pytest.raises(ValueError, match="NEWOW_PRODUCT_FREQUENCY_NOT_OPEN"):
            require_open_weekly_product(product)


def test_formal_rs_weekly_scope_uses_v2_and_keeps_other_seven_closed():
...
def test_formal_pk_weekly_scope_uses_v2_and_keeps_other_seven_closed():
