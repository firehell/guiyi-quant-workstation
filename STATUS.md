...
- production Alembic 仍在 `20260825_0040`，仓库 head 为尚未执行的 `20260826_0041`。HTDY production Scope 曾于 15:13 在独立明确授权下更新为 active 60 × 七周期 `60 symbols / 420 pairs`，随后于 20:43 按新的明确请求原子收敛为唯一 `jm × 15m`（`1 symbol / 1 pair`）；七周期图表能力与逐 `symbol × frequency` 开关能力不变，SuBing Scope 未变，未触发 Event、重放或通知。
...
- v1.8.5 与当前 Runtime 已退役 RQAlpha 与 Execution Review 的 active Web/API/domain/roll seam/tests/docs，Web 路由只保留 `/market` 与 `/market/chart`；SuBing Formal Event 与 Alert 主链保留。部署后 `/api/v1/backtests`、`/api/v1/execution-reviews` 与 `/api/v1/execution-review` 均为 404。仓库 Alembic head 已延伸到 `20260826_0041`，但 production 仍为 `20260825_0040`，四张 `trade_*` 表只读盘点均为 0 行，尚未执行删表。
...
- RQAlpha / Execution Review 退役代码已随 v1.8.5 切入 production Runtime 并读回健康状态；剩余 Gate 仅为 production `upgrade 20260826_0041` 的独立数据库写入授权与执行。外部 RQAlpha runs 根与遗留 sidecar 已关闭。
