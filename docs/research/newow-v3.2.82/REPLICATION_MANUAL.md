...
版本：2026-09-26 更新；当前详情 v3.3.59 / 首页 v3.3.65 / 选股页 v3.2.103；保留 v3.2.82 历史研究
定位：公开原式整理、股票页面复刻证据、当前差异与独立的期货适配建议
边界：研究观察；不是交易建议；不是模拟成交；不是实盘成交

> 本手册以公开页面、源码静态审计、实际交互、归一代码和历史证据为依据。原站公式以对应版本的公开函数/哈希为来源；归一代码是复刻对照，不替代原式权威。自有 clean-room、服务端未知、页面观察与同输入逐值通过分别标记。

## 当前手册导航（2026-09-26）

手册按三卷阅读，原版公式与期货建议不混写：

1. **[原版公式卷](FORMULA_CATALOG_20260926.md)**：集中查三策略、双策略融合、J/D1–D6/4-7-11、目标/吸筹、CDV2、AI六组合、四副图、形态、分时及收益统计。包含初始化、阈值、顺序、舍入和公开函数定位；本卷不加入期货修正。
2. **[最新功能与差异卷](AUDIT_20260926.md)**：当前功能库存、新旧变化、归一已实现与未实现、优化建议、新功能、UI交互、期货适配与验收待办。只登记，不开始开发。
3. **本文件下方历史研究卷**：保留既有公式解读、股票证据和期货研究；其中旧13格、OOS数字及过去实现状态都带历史身份，不作为当前页面规格或Runtime验收。

本轮关键结论：新增双策略单仓融合与双轨交互；CDV2和WR20内核与9月18日哈希相同；目标升级加入1.005缓冲。主升浪仅隐藏按钮，仍能从其他入口使用。趋势转折及专用副图现已被另一任务合入归一；新版CDV2、六组合、收益曲线和嵌套路径仍缺。

完整性边界：本轮已整理可公开核验的规则，尚未完成最新版全量同输入逐 Bar验收；服务端私有筛选、后端文本生成及实际灰度配置仍未知。页面参考收益、理论收益、期货因果研究和账户收益不共用口径。

### UI / 交互实现更新（2026-09-26）

本轮在单一 worktree 顺序完成九项展示与交互改动。逐项对照、截图和验证见 [九项交付记录](../../tasks/newow-ui-nine-20260926/README.md)。双策略仅提供同输入的双轨观察，不包含融合交易、融合收益或主导策略算法；公式速查是资料展示，不在前端重算。原式卷中相关算法的未实现边界仍然有效。

### 原式速查

| 查什么 | 阅读位置 |
|---|---|
| 黄蓝带、MA35/45、震荡触沿及同根处理 | [公式卷第3节](FORMULA_CATALOG_20260926.md#3-三个主策略与参考信号) |
| J、D1–D6、4/7/11 | [公式卷第4节](FORMULA_CATALOG_20260926.md#4-jd1d6-与-4711) |
| 双策略标注、主导和融合 | [公式卷第5节](FORMULA_CATALOG_20260926.md#5-新双策略三种不同的统计对象) |
| 普通/理论收益、回撤百分点、年化 | [公式卷第6节](FORMULA_CATALOG_20260926.md#6-收益胜率回撤理论值和年化) |
| 六组合AI与五窗口的区别 | [公式卷第7节](FORMULA_CATALOG_20260926.md#7-顶部-ai-六组合与五窗口比较器) |
| 目标/吸筹、通道与0.5%缓冲 | [公式卷第8节](FORMULA_CATALOG_20260926.md#8-目标价吸筹价与通道) |
| CDV2、R/MM、仓位、AI模板 | [公式卷第9节](FORMULA_CATALOG_20260926.md#9-综合决策-cdv2-120) |
| 四副图、攻击线、路径、形态、分时 | [公式卷第10–12节](FORMULA_CATALOG_20260926.md#10-四类副图及震荡辅助攻击线) |
| 首页独立显示、五个选股模板47项配置、覆盖度与消息边界 | [公式卷第14节](FORMULA_CATALOG_20260926.md#14-首页选股与官方说明的公开规则补充) |

以下为历史研究正文，公式发生新发现时以上方当前卷及精确来源为准。

<!-- PDF_PAGE -->

> 历史说明：2026-09-18时详情为v3.3.46、正式能力仅1d。2026-09-26代码能力已到1d/1w，趋势转折已有实现；旧缺项清单不再代表当前。详见[最新差异卷](AUDIT_20260926.md)。
...
18 类 feature 进入历史比较；其中通道、显示选择、五窗口排名、综合决策、方向/仓位、确定性、波动率和第一行动等 16 类均为 `27/27 matched`，总 mismatch 为 0。这个分母不包含所有主策略 bands/markers，也不是v3.3.59验收。AI 文案与诊断 token 没有稳定机器合同，各为 27 个 unavailable。
...
第一行动不是再算一个总分。历史实现按趋势周/日组合及震荡60分/日/周状态依次决定硬空、风险提示、等待或允许操作；没有把ATR波动率或S跑作为该函数输入。它们属于其他解释维度，不得补写成已有隐藏条件。最新版具体分支见[公式卷9.5](FORMULA_CATALOG_20260926.md#95-时间标签与第一行动原则)。
...
第三个分析 tab 改为“双策略”，保留主升浪内核及直接路由。主图自动显示趋势／震荡双轨；下方接入独立融合模型的三组统计、融合交易和建清仓来源。公式及期货换月、OPEN 合同不变。收益曲线、窗口快捷选择和融合理论值等完整产品工作拆分见[双策略设计与待办](../../tasks/newow-dual-entry-20260927.md)。此补充描述开发实现，不代表正式发布或 Runtime 验收。
