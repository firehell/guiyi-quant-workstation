...
- 当前可执行面：Web 为 Market；API/CLI 为 market / data CLI / runtime，并包含独立 Alert V1 代码面。Market Runtime V1 的仓库模板默认关闭；本地工作站已按明确请求启用由 `operational_products.txt` 界定范围的 Runtime。Alert Runtime 模板默认关闭，不能从 Market Runtime 授权推导启用。signal/review/strategy Web·HTTP·worker 与 data_center HTTP 已退役。
...
history 可追溯），当前不存在回测引擎、策略适配层或策略 HTTP/worker。Alert V1 的两张 Application
Domain 表不属于 Market Catalog，不改变八表合同。
...
### Alert Runtime V1 的独立受限持续授权

Alert 代码与 launchd 模板默认关闭。只有用户对识别出的本地工作站明确请求“启用 Alert Runtime V1”并
实际执行一次启用操作后，才允许持续处理 `htdy_original_15m × enabled scope_products × WeCom` 范围内
此后自然到达的 confirmed 15m Bar。停机历史不 replay/backfill，发送失败不 retry。

该授权与 Market Runtime V1 相互独立，不覆盖新增 Rule 或通知渠道、生产 migration、任何 Runtime switch、
main/tag/release、Canonical 写入或订单。production migration、真实 WeCom canary 和首次 Alert Runtime
activation 仍是三个独立的一次性 Gate；mock、测试、render-only 或其中一个 Gate 不授权其余 Gate。
...
8. 当前仓库不提供 backtest API/Web/worker/queue/CLI 或报告兼容入口。未来回测必须作为新任务基于 Canonical/MarketDataService 重建，并保留策略、参数、数据、订单、trade、equity 与 lineage 以支持复算。Signal/Review/Strategy HTTP·worker·DB 表与旧语义合同已退役；Alert V1 是独立 Application Domain，不以旧表或旧 worker 为入口；未来重建仍须新任务新合同。
9. live、Runtime promotion/switch、真实通知与企业微信 autosend 默认关闭；配置缺失、异常、过期或不一致时保持关闭。Market Runtime V1 的 Redis Live Overlay 与盘后 runner 只读取同一个 `operational_products.txt`，且不新增生产表。Alert V1 只处理启用 Scope 中自然到达的 `htdy_original_15m` confirmed Bar；repair、replay、backfill、migration 与 EOD recalculation 不补评或补发历史通知。
