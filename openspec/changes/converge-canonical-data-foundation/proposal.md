...
- **C2.5 Candidate target prerequisite**：Gate A 前将隔离 Candidate 收口为由 Catalog/Session/root/universe/floor/source policy/code SHA 组成的不可变 identity，并将 fixed through 单独记录为单调可扩展状态；仅允许严格 fresh 或 extend 校验后的 RQData-only update，禁止 reset/resume 操作。direct `1w` 请求显式映射为 provider weekly 请求；历史 Calendar/Session 事实与 Candidate 一起校验；诊断只输出字段白名单、固定原因码和受限样本的非敏感结构化摘要。
