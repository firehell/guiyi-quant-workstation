...
export function newowTrendDetailFixture({ product = 'jm', from = trendDays[0], through = trendDays.at(-1) } = {}) {
  ...
  return {
    ...
    price_channel: {
      daily: {
        ...
        points: bars.map((bar, index) => ({
          bar_end: bar.bar_end,
          target: index < 9 ? null : (trendCloses[index] + 2).toFixed(2),
          absorb: index < 9 ? null : (trendCloses[index] - 5).toFixed(2),
          window: 10, available: index >= 9,
          formula_version: 'newow_target_absorb_hhv_llv10_page_v1',
        })),
        owner_segment_ids: [segmentA, segmentB],
        formula_version: 'newow_target_absorb_hhv_llv10_page_v1',
      },
      weekly: {
        frequency: '1w', points: [{
          bar_end: bars[9].bar_end, target: '112.00', absorb: '96.00', window: 10,
          available: true, formula_version: 'newow_target_absorb_hhv_llv10_page_v1',
        }], owner_segment_ids: [segmentB], formula_version: 'newow_target_absorb_hhv_llv10_page_v1',
      },
      sixty_minute: {
        frequency: '60m', points: [{
          bar_end: bars[9].bar_end, target: '111.00', absorb: '98.00', window: 10,
          available: true, formula_version: 'newow_target_absorb_hhv_llv10_page_v1',
        }], owner_segment_ids: [segmentB], formula_version: 'newow_target_absorb_hhv_llv10_page_v1',
      },
      display: {
    ...
    page_window_comparison: [10, 20, 24, 30, 52].map((window, index) => ({
      window, cumulative_return_pct: String(10 - index), max_drawdown_pct: String(-2 - index),
      trade_count: 2 + index, win_rate_pct: '50', score: String(8 - index),
      terminal_position_was_open: false, force_closed_at_end: true,
    ...
    composite_cleanroom: {
      ...
      certainty: { trend: 20, oscillation: 20, alignment: 20, direction: 20, total: 80 },
      volatility: { value_pct: '1.80', level: 'low', sample_size: 9 }, risk_tokens: [],
      page_difference_reason: 'causal timing remains separate',
      formula_version: 'newow_composite_decision_cleanroom_v1',
    },
    ...
    diagnostic_facts: {
      as_of: bars[9].bar_end, target_price: '112.00', absorb_price: '96.00',
      target_distance_pct: '4.67', absorb_distance_pct: '-10.28', ema20: '105.00',
      close_vs_ema20: 'above', trend_state: 'YELLOW', trend_duration_bars: 4,
      oscillation_holding: true, main_force_status: '有庄控盘', main_rise_active: true,
      cup_state: 'WEAKENED', weekly_signal: 'hold', daily_signal: 'buy',
      repainting_inputs_excluded: ['newow_zhaoyao_mirror_repainting_page_v1'],
      formula_versions: ['newow_diagnostic_facts_cleanroom_v1', 'newow_diagnostic_rules_cleanroom_v1'],
    },
    diagnostic_tokens: [{
