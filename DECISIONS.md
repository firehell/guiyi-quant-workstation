...
| Newow 发布能力 authority | typed current/historical API 与 Web 共用无数据库依赖的 `product-capabilities`；develop 日版候选开放 `1w/1d` chart/auxiliary/reference/comparator | 未开放 `60m` 或 explanation 显式 409，不静默改写周期；旧 `/trend-detail` 固定 D1 兼容语义及其他 Market consumer 不受影响；解除 Gate 不等于数据、Release 或 Runtime 已通过 |
