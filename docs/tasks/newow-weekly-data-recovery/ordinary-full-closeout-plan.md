...
当前执行入口改为[证据丢失后的三项执行修订](remaining-123-implementation-plan.md)。本页保留总包设计与
历史记录；1,117/1,014 及旧批次不替代新完整审计。证据落在主仓库共同根，缺失旧来源证明时不直接排除
B2411、不恢复旧 attempt，也不把普通补数与专项异常混为一次未经界定的真实操作。
...
后续 owner 已确认有界工程修订：在新精确总包中，已证明零提交且来源响应/journal 完整的明确来源质量异常
可以隔离单元，并继续同批下一独立合约；额度、网络、锁冲突、漂移、提交/读回/清理未知仍全局停止。
旧 102 个成功单元必须在新完整原生 audit 中自然消失并逐项核对；若仍被提议，不得凭历史成功名单过滤，
须停止并保留当前缺口。B2411 凭完整历史证据与当前原生计划精确匹配后排除新下载，始终保留在未完成分母。
这项修订不改变下文旧执行的停批语义，不复用已消费的执行意图；新实现须完成测试、独立 Review、完整只读
重审及新包冻结，再取得新的精确执行意图。修订实现为 `208ce6550` 至 `eab1bc7ea`，主会话组合验证
609 passed，最终独立复审无剩余 finding，允许集成 develop。代码冻结检查点尚未执行新完整 audit/prepare
或真实执行；后续只读准备以本地 exact evidence 为准，仍须新的精确执行意图。
