...
from app.services.rqdata_ingest.jm_v2_parquet import CONTRACT, EXCHANGE, SYMBOL, evaluate_standard_dominant_quality
from app.services.rqdata_ingest.parquet import sha256_file


def register_jm_v2_quality(*, session: Session, summary_path: Path, manifest_path: Path | None = None) -> dict[str, Any]:
    summary = json.loads(summary_path.read_text(encoding="utf-8"))
    periods = summary.get("periods") or {}
    if not isinstance(periods, dict) or not periods:
        raise ValueError(f"JM v2 parquet summary has no periods: {summary_path}")
    manifest_path = manifest_path or summary_path.parents[3] / "manifests" / f"rqdata_jm_v2_history_{summary['start_date'].replace('-', '')}_{summary['end_date'].replace('-', '')}.csv"
    ...
    for period, payload in periods.items():
        ...
        if not standard_path.exists():
            raise FileNotFoundError(f"JM {period} standard parquet not found: {standard_path}")
        frame = pd.read_parquet(standard_path)
        quality = evaluate_standard_dominant_quality(frame, period)
        if quality.status == "failed":
            raise ValueError(f"JM {period} quality failed; refusing DB registration")
        checksum = sha256_file(standard_path)
        if checksum != standard["checksum"]:
            raise ValueError(f"JM {period} checksum mismatch: {checksum} != {standard['checksum']}")
