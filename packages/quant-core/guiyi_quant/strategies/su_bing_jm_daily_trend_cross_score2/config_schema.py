...
class SuBingJmDailyTrendCrossScore2Params:
...
def validate_params(raw_params: Mapping[str, Any] | None = None) -> SuBingJmDailyTrendCrossScore2Params:
    allowed = {field.name for field in fields(SuBingJmDailyTrendCrossScore2Params)}
    params = dict(DEFAULT_PARAMS)
    params.update({key: value for key, value in dict(raw_params or {}).items() if key in allowed})
    validated = SuBingJmDailyTrendCrossScore2Params(**params)
    ...
    if validated.require_trend_alignment is not True:
        raise ValueError("require_trend_alignment must stay true")
    if validated.require_macd_cross is not True:
        raise ValueError("require_macd_cross must stay true")
