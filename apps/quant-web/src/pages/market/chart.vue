...
watch(
  [visibleMainIndicators, selectedPeriod, realtimeFollowPreference],
  () => {
    saveMainChartPreferences({
      version: 1,
...
watch(
  ...
  () => {
    ...
    void loadMarketIndicators(marketRouteRequestId)
  },
  { deep: true },
)
      ...
      <section class="side-panel">
        ...
        <template v-if="hoverContext">
          <div class="snapshot-grid">
            ...
            <template v-for="indicator in hoverContext.mainIndicators || []" :key="indicator.id">
              <span>{{ indicator.displayName }}</span><strong>{{ formatNumber(indicator.value) }}</strong>
            </template>
