...
状态：`SLICE_D_IMPLEMENTED_REVIEW_PENDING`
...
证据清单 SHA-256：`2a3a699fda2e116020e1dea2648a9c5962eafb73bfb59b10978d63c63ea45c39`
...
| S跑与 D1-D3 | stock_detail.html v3.2.63 | REPRODUCED_EXACT | newow_escape_d123_page_v2 | packages/quant-core/guiyi_quant/newow/escape_d123.py | 9 标的旧金样本逐值 exact | actual_dominant D1 研究测试已存在 | Slice E 只读详情输出 |
| D4-D6 建仓标记 | stock_detail.html v3.2.63 | REPRODUCED_EXACT | newow_buy_d456_page_v1 | packages/quant-core/guiyi_quant/newow/main_rise.py | 9 标的旧金样本逐值 exact | actual_dominant D1 研究测试已存在 | Slice E 只读详情输出 |
...
| 主升浪 | stock_detail.html v3.2.63 | REPRODUCED_EXACT | newow_main_rise_ma35_ma45_page_v1 | packages/quant-core/guiyi_quant/newow/main_rise.py | 9 标的旧金样本 MA35/MA45、J、信号 exact | 因果 next-open 研究入口已存在 | Slice E 组合输出接入 |
...
| 11 周期 | stock_detail.html v3.2.63 | REPRODUCED_EXACT | newow_magic11_page_v1 | packages/quant-core/guiyi_quant/newow/magic11.py | 516 markers 与 3240 count lines exact | 主升浪组合内可复算 | Slice E 只读详情输出 |
...
| 涨跌动能副图 | stock_detail.html v3.2.63 | REPRODUCED_EXACT | newow_up_down_energy_page_v1 | packages/quant-core/guiyi_quant/newow/subplots.py | 9 标的逐值 exact | 研究 primitive 可复算 | Slice E 只读详情输出 |
...
| AI 周日 16 组合矩阵 | stock_detail.html v3.2.63 | CLEANROOM_IMPLEMENTED | newow_ai_week_day_16_matrix_page_v1 token projection | packages/quant-core/guiyi_quant/newow/diagnostic_rules.py::diagnostic_tokens | 当前 16 周日组合全部 token 分支测试 | 只作解释 token | Slice D 双路 Review |
| AI 六组合历史评分 | stock_detail.html v3.2.63 | REPRODUCED_EXACT | newow_ai_six_combo_page_v3_2_50 | packages/quant-core/guiyi_quant/newow/diagnostic_rules.py::rank_page_ai_combinations | 周/日/60m × 震荡/趋势；收益/Calmar/胜率、样本惩罚和稳定排序回归 | trustworthy_for_research=false，与 Walk-forward 返回类型隔离 | Slice D 双路 Review |
...
| 归一趋势建仓 candidate | 归一 clean-room v1 | CLEANROOM_IMPLEMENTED | newow_trend_build_candidate_v1 | packages/quant-core/guiyi_quant/newow/screener_observation.py::evaluate_trend_build_candidate | 同段黄带且最新 BUILD 晚于 CLEAR | 只处理传入 facts，page_parity=false | Slice D 双路 Review |
| 归一主升浪建仓 candidate | 归一 clean-room v1 | CLEANROOM_IMPLEMENTED | newow_mainrise_build_candidate_v1 | packages/quant-core/guiyi_quant/newow/screener_observation.py::evaluate_mainrise_build_candidate | 同段主升浪持有且最新 BUILD 晚于 CLEAR | 只处理传入 facts，page_parity=false | Slice D 双路 Review |
| 归一杯柄 candidate | 归一 clean-room v1 | CLEANROOM_IMPLEMENTED | newow_cup_handle_candidate_v1 | packages/quant-core/guiyi_quant/newow/screener_observation.py::evaluate_cup_handle_candidate | READY/BREAKOUT 且无 hard failure | 只处理传入 facts，page_parity=false | Slice D 双路 Review |
