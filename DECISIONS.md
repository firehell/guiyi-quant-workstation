...
| Newow 发布能力 authority | typed current/historical API 与 Web 共用无数据库依赖的 `product-capabilities`；v1.10.12 Release 仅开放 `1d` chart/auxiliary/reference/comparator | 未开放 `1w`、`60m` 或 explanation 显式 409，不静默改写周期；旧 `/trend-detail` 固定 D1 兼容语义及其他 Market consumer 不受影响；Release 不授权 Runtime promotion 或推定数据缺口已修复 |
