...
def test_pending_open_applies_on_exact_first_completed_1m() -> None:
    ...
    state, output = step_subing_strategy_machine(state, Completed1mBar(first))
...
def test_later_1m_cannot_substitute_for_missing_first_bar() -> None:
    ...
    state, output = step_subing_strategy_machine(state, Completed1mBar(later))
...
def test_equal_boundary_message_order_is_invariant() -> None:
    ...
    first, first_output = step_subing_strategy_machine(state, completed_15m)
    first, second_output = step_subing_strategy_machine(first, completed_5m)
    second, third_output = step_subing_strategy_machine(state, completed_5m)
    second, fourth_output = step_subing_strategy_machine(second, completed_15m)
...
def test_missing_equal_boundary_companion_fails_closed_at_next_boundary() -> None:
    state, _, effective = _machine()
    state, output = step_subing_strategy_machine(state, Completed15mBar(effective))
    ...
    with pytest.raises(SubingStrategyMachineError, match="BOUNDARY_COMPANION_MISSING"):
        step_subing_strategy_machine(state, Completed5mBar(later))


def test_duplicate_is_idempotent_and_conflicting_duplicate_fails_closed() -> None:
    state, first, _ = _machine()
    state, _ = step_subing_strategy_machine(state, Completed1mBar(first))

    duplicate, output = step_subing_strategy_machine(state, Completed1mBar(first))
    ...
    with pytest.raises(SubingStrategyMachineError, match="CONFLICTING_DUPLICATE"):
        step_subing_strategy_machine(state, Completed1mBar(conflicting))


def test_stale_completed_bar_is_rejected() -> None:
    state, first, _ = _machine()
    later = replace(first, bar_end=first.bar_end + timedelta(minutes=1))
    state, _ = step_subing_strategy_machine(state, Completed1mBar(later))

    with pytest.raises(SubingStrategyMachineError, match="STALE_INPUT"):
        step_subing_strategy_machine(state, Completed1mBar(first))
...
def test_terminal_close_uses_completed_15m_close_without_open_timestamp() -> None:
    state, first, effective = _machine(pending=True)
    state, _ = step_subing_strategy_machine(state, Completed1mBar(first))
    state, _ = step_subing_strategy_machine(state, Completed15mBar(effective))
    state, _ = step_subing_strategy_machine(state, Completed5mBar(effective))
