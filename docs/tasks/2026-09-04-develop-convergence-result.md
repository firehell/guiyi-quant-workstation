...
- Conflict inventory：`TESTING.md=MANUAL-MERGE`；`docs/research/newow-v3.2.82/README.md=AUTO-MERGE`；`.gitattributes`、`REPLICATION_MANUAL.md`、PDF、构建脚本和 `tools/docs` 锁文件仅由 `15a5576` 引入，均为 `NO-OVERLAP`；其余 convergence 文件同样为 `NO-OVERLAP`。手册与 PDF 的 Owner approval 更新属于 merge 后 semantic reconciliation，不冒充 merge conflict。
...
- `uv sync --project tools/docs --locked` exit 0；锁定环境 resolved 4 / checked 3。手册 canonical rebuild 命令 exit 0，生成 45 页 A4 PDF；连续两次重建 SHA-256 均为 `3c6881db880fdb87a11763882214a5390ecbe371d60936de99ce2bc8f5fb21d5`，证明生成结果确定。构建器不再试探多个宿主字体或退回 Helvetica；它只接受 `/System/Library/Fonts/STHeiti Medium.ttc` 且必须精确匹配 SHA-256 `f8fa4a63e2cf500e98e64d4c73260daaba049306cf85dec9e3729bc285b7d645`，缺失或不匹配均 fail-closed。PDF 全 45 页 contact-sheet 与末页原尺寸 visual QA 未见裁切、重叠、乱码或失效图片；bundled `pypdf` readback 为 45 页，并检出两条 Owner 决定 identity 与“29 张冻结截图已经取得 Owner”文案。系统缺少可选 `pdftotext`，该命令返回 `command not found` 后改用 bundled `pypdf`，不影响 canonical rebuild 结果。
