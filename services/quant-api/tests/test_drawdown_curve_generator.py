...
def test_generates_drawdown_curve_from_running_peak() -> None:
    ...
    assert result["drawdown_curve"] == [
        ...
        },
        {
            ...
            "peak_equity": 101000.0,
            "drawdown": 3000.0,
            "drawdown_pct": 3000.0 / 101000.0,
            "source_trade_id": "T3",
        },
    ]
    assert result["max_drawdown_amount"] == 3000.0
    assert result["max_drawdown_pct"] == 3000.0 / 101000.0
...
def test_new_high_resets_drawdown_before_later_loss() -> None:
    ...
    assert [point["peak_equity"] for point in result["drawdown_curve"]] == [1000.0, 1000.0, 1100.0, 1100.0]
    assert [point["drawdown"] for point in result["drawdown_curve"]] == [0.0, 100.0, 0.0, 55.0]
