...
def test_rqdata_bar_adapter_normalizes_exchange_daily_bar_end(tmp_path) -> None:
    session, _starts = _session(tmp_path)
    ...
    client = ExchangeDailyClient({"JM2509": frame})
    adapter = RQDataMarketAdapter(session=session, client=client)
    key = DatasetKey("contract", "jm", "JM2509", "1d")
...
def test_rqdata_daily_adapter_uses_exchange_daily_zero_trade_ohlc(tmp_path) -> None:
    """期货日线必须保留交易所零成交日的原始 0 OHLC，而非将结算价写为 close。"""
    session, _starts = _session(tmp_path)
    expected = datetime(2025, 1, 6, 1, 5, tzinfo=UTC)
...
def test_rqdata_daily_and_weekly_normalize_zero_volume_zero_ohl_to_close(
    tmp_path,
) -> None:
    """RQData 的非零 close 配合 0 O/H/L 是零成交占位，不是有效零价。"""
    session, _starts = _session(tmp_path)
    daily_key = DatasetKey("contract", "jm", "JM2509", "1d")
    weekly_key = DatasetKey("contract", "jm", "JM2509", "1w")
    daily_ends = tuple(
...
def test_rqdata_weekly_continuous_aggregates_daily_rank1_segments(tmp_path) -> None:
    ...
    session, _starts = _session(tmp_path)
    for day, contract in ((6, "JM2509"), (7, "JM2509"), (8, "JM2509"), (9, "JM2505"), (10, "JM2505")):
        session.add(
            MainContractMap(
                symbol="jm",
                trade_date=date(2025, 1, day),
    ...
    client = ExchangeDailyClient(
        {"JM2509": pd.DataFrame(first_rows), "JM2505": pd.DataFrame(second_rows)}
    )
    adapter = RQDataMarketAdapter(session=session, client=client)
...
def test_rqdata_weekly_adapter_rejects_incomplete_exchange_daily_week(tmp_path) -> None:
    """周线缺少任一交易所日事实时不得以部分周生成 bar。"""
    session, _starts = _session(tmp_path)
    expected = datetime(2025, 1, 10, 1, 5, tzinfo=UTC)
    rows = [
        {
...
def test_continuous_main_does_not_fall_back_from_88_to_99(tmp_path) -> None:
    ...
    index_frame = pd.DataFrame(
        [
            {
                "datetime": datetime(2025, 1, 6, 9, 5),
                "trading_date": date(2025, 1, 6),
                "open": 100,
                "high": 101,
                "low": 99,
...
def test_current_day_metadata_snapshot_fetches_iso_week_period_evidence() -> None:
    period_calls = []
    current_day = date(2026, 9, 21)
    next_trading_day = date(2026, 9, 22)
    week_end = date(2026, 9, 27)
    later_trading_day = date(2026, 9, 23)
    trading_days = tuple(current_day + timedelta(days=offset) for offset in range(5))
    ...
    class Api:
        ...
        def get_trading_dates(self, start_date, end_date):
            return tuple(day for day in trading_days if start_date <= day <= end_date)

        def get_trading_periods(self, order_book_ids, start_date, end_date, frequency):
            period_calls.append((tuple(order_book_ids), start_date, end_date, frequency))
            days = [day for day in trading_days if start_date <= day <= end_date]
            return pd.DataFrame(
    ...
    assert period_calls == [(("JM2509",), current_day, week_end, "1m")]
    assert {day for _, day, _ in snapshot.main_contracts} == {
        current_day,
        next_trading_day,
    }
    assert later_trading_day in {row["effective_from"] for row in snapshot.sessions}
    assert any(
        row["trade_date"] == later_trading_day and row["has_night_session"] is True
...
def test_current_day_metadata_snapshot_preserves_missing_next_period_for_sync() -> None:
    ...
    assert snapshot.main_contracts == (
        ("jm", current_day, "JM2509"),
        ("jm", next_trading_day, "JM2509"),
    )
    assert {row["effective_from"] for row in snapshot.sessions} == {current_day}


def test_current_day_metadata_snapshot_does_not_mask_missing_current_rank1() -> None:
    current_day = date(2025, 1, 10)
    next_trading_day = date(2025, 1, 13)
    ...
    class FuturesApi:
        def get_dominant(self, underlying_symbol, start_date, end_date, rule=0, rank=1):
            return pd.Series(
                ["JM2509"],
                index=pd.to_datetime([next_trading_day]),
                name="dominant",
            )
    ...
    assert snapshot.main_contracts == (("jm", next_trading_day, "JM2509"),)
    assert snapshot.sessions == ()


def test_current_day_metadata_snapshot_preserves_duplicate_current_rank1() -> None:
    current_day = date(2025, 1, 10)
    next_trading_day = date(2025, 1, 13)
    ...
    class FuturesApi:
        def get_dominant(self, underlying_symbol, start_date, end_date, rule=0, rank=1):
            return pd.Series(
                ["JM2509", "JM2509", "JM2509"],
                index=pd.to_datetime([current_day, current_day, next_trading_day]),
                name="dominant",
            )
    ...
    assert snapshot.main_contracts.count(("jm", current_day, "JM2509")) == 2
    assert {row["effective_from"] for row in snapshot.sessions} == {current_day}
