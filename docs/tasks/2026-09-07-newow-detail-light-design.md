...
参考区允许独立蓝色“空仓等待中”状态卡：只使用兼容、ready、当前 chart 的最新 completed FLAT frame，历史模式、历史 viewport、stale 或同 owner OPEN 冲突时不显示。等待卡不从分页列表猜测最近清仓记录，也不附带未经精确关联的收益。日期展示日内为上海时间 MM-DD HH:mm，跨年保留年份；日线/周线为 YYYY-MM-DD；原始时间和身份保留在详情中。
