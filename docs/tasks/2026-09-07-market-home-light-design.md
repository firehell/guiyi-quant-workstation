...
本文保留批准的视觉来源与显示细节；实现步骤已完成并从当前文档删除，历史见 Git。业务协议以 `openspec/specs/market-home-overview/spec.md` 为准，Release 与 Runtime 以 `STATUS.md` 为准。
...
首页采用局部白色样式；已替代的深色草案不再作为实现来源。首页样式不得改变其他视角的图表主题或全局指标颜色。
...
局部前缀 `--gy-home-*`，根背景 `#FFFFFF`，正文 `#20242B`，次级 `#667085`，分隔 `#EBEDF0`，表头 `#F8F9FB`。选中/强调 `#FF6B2C`；hover `#FFF6F0`。涨 `#FF403A`，跌 `#22B95D`，涨/跌软底 `rgba(255,64,58,.10)` / `rgba(34,185,93,.10)`。文字不随父容器 opacity 变淡。图标维持原状态映射，首页红/绿圆局部沿用上述涨跌色，蓝圆局部使用 `#365AF5`，不改变共享默认值或图表标记。

数字 tabular-nums；现有 numeric close 原值不再固定截成两位小数，格式化最多保留现有 Number 可表达的小数精度，不新增金额计算。涨跌幅/增仓率展示带符号两位百分数，null 为 `—`，0 为 `0.00%`。量比作为展示比率保留两位小数，不沿用收盘的完整精度格式。标签圆角 7px，padding 3px 10px；图标 28px，方向 micro 24px，充分对齐。
