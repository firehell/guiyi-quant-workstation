...
- 当前 `signal_events` 已具备显式 contract context 字段，Stage 9 仍需实现 payload、guarded sender、通知记录和失败重试。
- 企业微信只读提醒、全品种 active Gate 审计、长期 worker/scheduler、阿里云 Web 托管验收未完成。
- V1 不做自动下单，不接实盘交易。
