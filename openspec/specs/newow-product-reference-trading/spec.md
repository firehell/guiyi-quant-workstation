...
全部 60 品种开放 `1d`，并对版本化 59 品种开放 `1w` 的
chart/auxiliary/reference/comparator；其余 1 品种的 `1w` 返回
...
正式日周版使用 `newow_product_capabilities_v21`、`daily_weekly` 声明 `1d/1w`，并携带唯一的
`weekly_products`。b、bz、cj、eb、eg、j、oi、pf、pg、pk、pl、pr、px、rs、sf、sh、si、sr 在正式周线请求上使用 `newow_weekly_input_quality_v2`，
...
其中尚未正式开放的 1 个品种仍只在候选请求中进入 reader，正式请求返回
`NEWOW_PRODUCT_FREQUENCY_NOT_OPEN`。
正式 v21 合同只有在独立 Release 与 Runtime promotion 完成后才成为现场事实。历史 v20、v19、v18、v17、v16、v15、v14、v13、v12、v11、v10 与 v8
分别表示先前的 58、57、56、55、54、53、52、51、50、49、48 与 41 品种正式周线。
...
- **GIVEN** 当前 capability 的 `release_stage=daily_weekly`、`open_frequencies=["1d","1w"]`
  及固定 `weekly_products`
- **WHEN** 客户端以正式 `weekly_products` 之外的品种请求 `1w`，或任意品种请求 `60m`
