...
| 发布与部署边界 | `v1.10.3` 已发布；用户随后明确授权部署。新 Runtime `/Volumes/扩展盘/guiyi-quant-runtime-v1.10.3-r1@dd9f3fa0fa332d433e4409dd657d35174b2b371c` 已创建为 clean detached，独立 Python/Web 依赖和构建、render-only 均完成，但五项服务仍运行 `v1.10.0`。部署准备中，盘后状态复制脚本因品种清单路径写错而在创建目标文件前退出；未执行任何服务切换或新 activation marker 写入。按单次执行意图规则停止，状态为 `BLOCKED / EXTERNAL_GATE_PENDING`，不得声明 v1.10.3 已部署或 `RUNTIME_READY`。 |
...
| Runtime root 核对 | 发布后的两个 RC 工作树与分支已清理。部署准备后现保留主 develop、clean detached v1.10.3 待部署根、五服务现役 `v1.10.0@f8f7d91765122c33cf5e82ed425b6c44f41ad0b1` 以及 clean detached `v1.9.15@36fef03923a168145e6fd2eab023dc1d2b411ad6` 旧回滚根。失败后的只读核对确认五服务 root/loaded commit 未变，新根的 after-market status 和两个 activation marker 均不存在；未回滚、未重试写入、未删除现役或恢复目录。 |
