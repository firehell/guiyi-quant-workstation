"""Exact PK W1 missing-week and receipt-backed turnover repair."""
...
PREPARED = HERE / "pk-w1-prepare.json"
...
def load_targets():
    ...
    if (audit["products"] != ["pk"] or audit["revision_stable"] is not True
            or audit["provider_requests"] != 0 or audit["writes"] != 0
            or len(audit["contracts"]) != 18
    ...
    if (dict(counts) != {"W1_MISSING_D1_COMPLETE": 149,
                         "D1_W1_VALUE_CONFLICT": 4,
                         "PROVEN_QUALITY_INTERRUPTION": 1}
            or set(targets) != {"PK2411", "PK2511", "PK2603", "PK2611"}):
        raise ValueError("ISSUE_COUNTS_MOVED")
    receipts = json.loads(RECEIPTS.read_bytes())
    evidence = {}
    for row in receipts["rows"]:
        if row["symbol"] != "pk" or not row["all_daily_receipted"]:
            continue
            ...
            for receipt_path in part["matching_receipts"]:
                ...
                if (receipt["status"] != "passed" or receipt["contract"] != row["contract"]
                        ...
                        or not any(item["year"] == part["year"] and item["month"] == part["month"]
                                   and item["dataset"] == ["contract", "pk", row["contract"], "1d"]
...
def build_packet(session, root: Path, config_identity):
    ...
    for contract in sorted(targets):
        d1_key = DatasetKey("contract", "pk", contract, "1d")
        w1_key = DatasetKey("contract", "pk", contract, "1w")
        ...
        for issue in targets[contract]:
            ...
            weekly_points = [point for point in mds.expected_contract_replay_endpoints(
                symbol="pk", contract=contract, frequency=w1_key.frequency,
            ...
            expected = tuple(point for point in mds.expected_contract_replay_endpoints(
                symbol="pk", contract=contract, frequency=d1_key.frequency,
    ...
    counts = {"insert": sum(change["action"] == "insert" for row in months for change in row["changes"]),
              "replace_turnover": sum(change["action"] == "replace_turnover" for row in months for change in row["changes"])}
    if len(months) != 40 or counts != {"insert": 149, "replace_turnover": 4}:
        raise ValueError("BATCH_COUNTS_MOVED")
    packet = {"schema_version": "pk_w1_missing_and_turnover_prepare_v1",
...
def mds_readback(catalog, store, packet):
    ...
    for record in packet["months"]:
        for change in record["changes"]:
            targets[record["contract"]].add(date.fromisoformat(change["day"]))
    ...
    for contract, days in targets.items():
        bars, gaps = mds.query_contract_weekly_replay_quality(
            symbol="pk", contract=contract, through=max(days), cutoff=cutoffs[contract],
    ...
    if interruptions != {("PK2611", date(2025, 12, 19))}:
...
def main():
    ...
    try:
        ...
        if (packet["schema_version"] != "pk_w1_missing_and_turnover_prepare_v1"
                ...
                or packet["month_count"] != 40
                or packet["week_counts"] != {"insert": 149, "replace_turnover": 4}
        ...
        if args.mode == "inspect":
            with Session(engine, autoflush=False) as session:
                ...
                states = []
                for record in packet["months"]:
                    key = DatasetKey("contract", "pk", record["contract"], "1w")
        ...
        if args.mode == "dry-run":
            ...
            print(json.dumps({"status": "dry_run_passed", "sha256": sha(raw),
                              "months": 40, "weeks": packet["week_counts"], "writes": 0}))
            return
        with Session(engine, autoflush=False) as session:
            catalog = MarketCatalog(session, root)
        ...
        print(json.dumps({"status": "committed", "sha256": sha(raw),
                          "months": 40, "weeks": packet["week_counts"], "readback": "passed"}))
    finally:
        engine.dispose()
