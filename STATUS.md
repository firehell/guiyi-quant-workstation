...
- 严格只读自然数据验收使用 PostgreSQL `READ ONLY` transaction + 最终 rollback，且注入 `NullSubingStrategyCache`；未调用 RQData、HistoricalDataManager、Redis、Alert、PushPlus 或任何写入路径。Review 修正后 AG/JM/RB 分别得到 `66/31/27` 个完整 Episode，自然覆盖 long/short、四种 `ENTRY_CONFIRMED` 来源、EMA21/上一根极值/MACD 三类退出、同 Bar 多原因、session/overnight gap 和窗口外建仓/窗口内清仓。额外找到 EG2609 在 2026-08-21 的真实段末 Episode，以旧段最后 15m close 清仓并保留全部 reason codes。对 AG/JM/RB/EG 共 272 个 Action 的同合约 next-open/terminal-close、可用退出原始值、MACD 位阶与单 effective-Bar 唯一性逐项检查全部通过。
- 关闭策略缓存后，Review 修正后的同一 JM 单段请求连续 5 次均为 `1 segment / 3057 5m / 1019 15m / 2 Actions / 1 Episode / cache_state=unavailable`，身份与结果完全一致，耗时 `3.473/3.400/3.347/3.387/3.379s`，median `3.387s`；不设性能阈值。真实 rollover 验收曾发现并以 RED→GREEN 修正两个边界：新段首日允许上一交易日方向 context 指向旧合约，以及普通 `through` 查询的观测前缀不必与权威物理段末完全相等。
- 分支最终自动验证为 Python `2833 passed / 3 skipped / 1 deselected`、Ruff clean、Mypy `151 source files`、canonical consistency `8 passed`、Web unit `288 passed / 1 skipped`、Vue typecheck + production build/bundle topology 通过、Web E2E `77 passed`。OpenSpec strict、secret scan 与 diff 检查结果以 PR exact head 的最终读回为准。
- PR #225 首次 exact-head 独立 Review 发现：既有 Lifecycle 的多头突破高点/空头突破低点被错误当成批准设计所需的多头绑定低点/空头绑定高点，另有平仓 Marker 位置和 Tooltip 事实缺口。代码已按 RED→GREEN 修正：非定向结构 Pivot 不再被重标，只记为 `structure_exit_available=false`，建仓仍保留；Marker 位置与事实字段已修正。第二次 Review 进一步发现开仓 Tooltip 读取了 Episode 结束后才产生的持有期/参考变动、平仓 Tooltip 缺少入场端事实；现已按 Action 类型拆分，Entry 只显示当时已知事实，Exit 显示入场/平仓两端事实和最终结果。修正后四品种 `structure_exit_available=0`、合法 bound-low/bound-high 自然退出数为 0，说明当前权威 Lifecycle 事实没有提供批准 Spec 所需的结构 Pivot；不得用突破 Pivot 冒充或隐式搜索替代。
- 该分支尚未合入 `develop`，未进入 release/main/Runtime，未创建 migration、Stage 2、SuBing Strategy Alert Rule、账户/订单/费用/资金曲线或真实通知。因缺少权威结构 Pivot 来源与自然退出案例，当前集成 Gate 为 `阻塞`；即使同一独立 Review 对修正后 exact head 无新代码 finding，也不得自动合并。
