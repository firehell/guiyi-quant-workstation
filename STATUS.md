...
| 发布与部署边界 | `v1.10.3` 已发布，API/Web 身份均为 `1.10.3`。本轮按用户要求停在服务部署之前，未创建新 Runtime root、未切换五项服务、未变更 activation marker；正式运行版本仍为 `v1.10.0`，不得宣称 v1.10.3 已上线或 `RUNTIME_READY`。 |
...
| Runtime root 核对 | 2026-09-08 发布后只读核对五项服务的 root/loaded commit 仍为 `guiyi-quant-runtime-v1.10.0-r1@f8f7d91765122c33cf5e82ed425b6c44f41ad0b1`。已删除 clean、完整合入 develop/main、未被服务引用的 `.worktrees/release-v1.10.2@2111395f...` 与 `.worktrees/release-v1.10.3@9f22847cf...`，以及对应本地/远端候选分支；需要时可由保留的合并 commit 重建。现仅保留主 develop、clean detached v1.10.0 现役根和 clean detached `v1.9.15@36fef03923a168145e6fd2eab023dc1d2b411ad6` 回滚根。 |
