...
OLD=P.parent/'2026-09-10'/'analysis.json'
source=json.loads((P/'source.json').read_text());market=json.loads((P/'market_history.json').read_text());prev=json.loads(OLD.read_text());old={r['id']:r for r in prev['records']};SH=ZoneInfo('Asia/Shanghai')
...
records=[];changes=[];new_mature=[];recheck_unavailable=[]
for ev in source['events']:
 ...
 r['cohort']='当日新增' if r['trading_day']==source['today'] else '此前已有' if r['id'] in old else '其他新增'
 ...
 for h in [15,30,60]:
  ...
  r['previous_return_'+k]=old.get(r['id'],{}).get('return_'+k)
  if not r['notification_attempted_at']:st='无发送尝试时间'
  elif r['side'] is None:st='方向冲突'
  elif not times:st='数据不足'
  elif anchor_i+h>=len(times):st='数据不足' if m.get('current_day_endpoints_unavailable') else '待观察'
  ...
  if r['id'] in old:
   ...
   elif oval is not None and nval is None:recheck_unavailable.append({'id':r['id'],'horizon':h,'previous':oval,'current_status':st})
   elif oval is not None and D(oval)!=D(nval):changes.append({'id':r['id'],'horizon':h,'previous':oval,'current':nval})
