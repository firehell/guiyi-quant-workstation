...
| 21 品种精确退役 | 活动品种池收口为 69 个；JR/PM/RI/WH/ZC/WR/BB/FB/PP_F/L_F/V_F/BC/CY/LG/AD/OP/RR/T/TF/TS/TL 进入一次性全链路退役 | 只允许 `GY-DATA-PRODUCT-RETIREMENT-21` 的 exact packet、独立 Review、owner exact-hash approval、69 品种 Runtime 与停服 receipt 后删除；当前代码/计划不授权真实 DML、文件删除、release 或 Runtime |
...
| 历史工件受控删除 | evidence/report/receipt 默认保护；只允许 exact deletion manifest + zero active references + independent Sol Review + owner exact-scope approval 后的受控删除 | `GY-DATA-PRODUCT-RETIREMENT-21` 是唯一新增窄例外；Git 历史不重写，混合证据只删目标行，并保留最小最终退役 receipt；本决策本身不授权 apply |
