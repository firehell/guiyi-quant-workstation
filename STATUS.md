...
- Stage 1 Historical Strategy Projection 已由 PR #225 合入 `develop`；批准后的 Lifecycle 结构绑定修正在隔离分支 `research/subing-strategy-v1` 与修正 PR #226。能力仍只支持 active 单品种 `actual_dominant + 15m`，从真实 rank1 物理段起点复算 Daily Watch V2 方向、Factor、Lifecycle、Action 与 Episode；Web 只显示“历史因果投影 / 模拟动作 / 非实际成交 / 参考变动”。
- Lifecycle 已把两种 Pivot 角色原子分开：既有 `trigger_reference_pivot` 继续是多头 HIGH / 空头 LOW 的突破、回踩与 risk 事实；`bound_reference_pivot` 是在唯一 `ENTRY_CONFIRMED` 冻结的同交易日、同物理段、严格 `confirmed_at < confirmation boundary` 的最新 5m `ConfirmedPivot`，多头绑定 LOW、空头绑定 HIGH，不设置 opportunity-origin 下界。缺失时保持 `None` 且不阻止建仓。完整 Lifecycle formula 为 `subing_lifecycle_v2_structure_binding_v1`；Candidate Validation / Robustness 只保留其实际消费且未变化的 scoped projection identity。
- fresh 严格只读自然验收窗口为 `2024-01-01..2026-08-25`，PostgreSQL `transaction_read_only=on`、最终 rollback，并显式注入 `NullSubingStrategyCache`；未构造真实策略缓存，也未调用 RQData、HistoricalDataManager、Redis、Alert、PushPlus 或任何写入路径。AG/JM/RB/EG 分别得到 `89/58/48/58` 个完整 Episode，共 `253` 个；`211` 个 Entry 携带合法 bound Pivot，自然出现 `BOUND_LOW_PIVOT_BREACH=8` 与 `BOUND_HIGH_PIVOT_BREACH=7`。
- 自然语料同时覆盖 long `131`、short `122`、四种 `ENTRY_CONFIRMED` 来源、EMA21/上一根 15m 极值/绑定 Pivot/MACD 四类退出、`47` 个同 Bar 多原因、`41` 个 session/overnight gap、EG2609 权威段末 close，以及 `4` 个窗口外建仓/窗口内清仓且完整 Episode 相等的 pan/prepend 案例；必需自然案例缺口为空。每个 Action 已对照同合约 Canonical next-Bar open 或 terminal close，每个退出已对照原始 Factor Frame、reason 顺序与单 effective-Bar 唯一性。
- 缓存关闭下同一 JM 请求连续 5 次均为 `2 segments / 6093 5m / 2031 15m / 12 Actions / 6 Episodes / cache_state=unavailable`，结果 shape 一致，耗时 `7.120/7.112/7.224/7.036/7.290s`，median `7.120s`；未设置或声明性能阈值。缓存仍是 observation base 下可删除、非权威的 `cache/subing-strategy-v1/`，任何缓存失败不得改变计算结果。
- 修正代码 head `c97b2fa26` 的 fresh 自动验证为 Python `2852 passed / 3 skipped / 1 deselected`、Ruff clean、Mypy `151 source files`、canonical consistency `8 passed`、Web unit `291 passed / 1 skipped`、Vue typecheck + production build/bundle topology、Web E2E `77 passed`、OpenSpec strict `5 passed`、secret scan `0 findings` 与 diff clean。原独立 Lane 3 Review 的 Critical/Important/Minor 均为零，权威 Pivot 与自然退出 blocker 已解除，结论为 `允许集成 develop`。
- 修正 PR #226 仍为人工集成 Gate，不得自动合并；未进入 release/main/Runtime，未创建 migration、Stage 2、SuBing Strategy Alert Rule、账户/订单/费用/资金曲线或真实通知。下一步仅允许人工决定是否把 PR #226 集成到 `develop`，不能由本 Gate 推导 release、tag、Runtime switch、Scope、通知或订单授权。
