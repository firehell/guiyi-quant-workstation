...
| Newow 发布能力 authority | typed current/historical API 与 Web 共用无数据库依赖的 `product-capabilities`；正式 `newow_product_capabilities_v22` 日周版对 60 品种开放 `1d/1w` chart/auxiliary/reference/comparator。b、bz、cj、eb、eg、j、oi、pf、pg、pk、pl、pr、px、rs、sf、sh、si、sm、sr 的正式周线使用 weekly v2 输入政策，其余 41 品种保持 v1 | 全部 `60m` 和 explanation 显式 409，不静默改写周期；旧 `/trend-detail` 固定 D1 兼容语义及其他 Market consumer 不受影响；Release 不授权 Runtime promotion 或推定数据缺口已修复 |
