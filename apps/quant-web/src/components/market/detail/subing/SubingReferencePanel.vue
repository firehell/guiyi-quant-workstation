...
<template>
  <section class="subing-reference" aria-label="历史参考交易">
    ...
    <template v-if="data">
      ...
      <div class="subing-reference__table"><table><thead><tr><th>方向 / 状态</th><th>物理合约</th><th>开仓参考</th><th>平仓参考</th><th>持有 Bar</th><th>参考收益 / 浮动</th><th>操作</th></tr></thead><tbody><tr v-for="trade in data.items" :key="trade.reference_trade_id"><td>{{ trade.side === 'LONG' ? '多' : '空' }} · {{ statusLabel(trade) }}{{ trade.initial ? ' · 窗口初始' : '' }}</td><td>{{ trade.physical_contract }}</td><td>{{ formatMarketDecimal(trade.entry_reference_price) }}<small>{{ formatBeijingInstant(trade.entry_bar_end) }}</small></td><td>{{ formatMarketDecimal(trade.exit_reference_price) }}<small>{{ (trade.exit_bar_end ? formatBeijingInstant(trade.exit_bar_end) : null) ?? (trade.status === 'ROLLOVER_INTERRUPTED' ? '换月中断，未配对平仓' : '尚无反向信号') }}</small></td><td>{{ trade.holding_bars }}</td><td :class="referenceTone(trade.reference_return_pct ?? trade.mark_change_pct)">{{ trade.reference_return_pct !== null ? `${referenceDecimalDisplay(trade.reference_return_pct)}%` : trade.mark_change_pct !== null ? `${referenceDecimalDisplay(trade.mark_change_pct)}% · 参考浮动` : '—' }}</td><td class="subing-reference__actions"><button type="button" @click="emit('focus', trade)">定位图表</button><button type="button" @click="emit('details', trade)">查看详情</button></td></tr></tbody></table></div>
      ...
      <details><summary>参考来源与版本</summary><p>{{ data.performance_since }} — {{ data.performance_through }} · 截止 {{ formatBeijingInstant(data.reference_cutoff) }}</p><p>{{ data.formula_version }} · {{ data.reference_model_version }}</p><p>快照 {{ data.input_snapshot_hash }}</p><p>读取截至 {{ formatBeijingInstant(data.as_of) }} · 历史回放 · 不可执行 · 不自动下单</p></details>
    </template>
  </section>
</template>
<style scoped>
