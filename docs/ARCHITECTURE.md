...
当前不是从零开始，而是在已有 MVP 上收敛重构。旧的 V1-A “焦煤 1 年验收样板”和历史 V1-B / V1-Final 结果只作为历史资产、回测口径参考和可复用实现依据，不作为当前继续扩展自动交易的理由。
...
- 阶段 1 先做 RQData 权限与接口能力 PoC。
- PoC 通过后再推进 JM 数据更新到最新交易日。
- 再收敛 manifest / checksum / quality_status。
- 实时 1m 入库、`signal_events`、企业微信只读提醒和 Web Market 策略展示都是后续任务，不是当前已完成能力。
- 信号只提醒，不自动下单。
