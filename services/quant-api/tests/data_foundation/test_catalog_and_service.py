...
def test_continuous_and_contract_query_use_catalogued_physical_partitions(session, tmp_path) -> None:
    catalog = MarketCatalog(session, tmp_path)
    store = CanonicalMonthlyStore(tmp_path)
    ...
    result = service.query(SeriesQuery(
        "contract", "jm", "1d", datetime(2025, 1, 1, 7, tzinfo=UTC),
        datetime(2025, 1, 3, 7, tzinfo=UTC), "JM2505",
    ))
...
def test_query_reads_across_monthly_physical_partitions(session, tmp_path) -> None:
    ...
    result = MarketDataService(catalog, store).query(SeriesQuery(
        "continuous", "jm", "1d", datetime(2025, 1, 30, 7, tzinfo=UTC),
...
def test_derived_query_reads_derived_partition_without_1m_fallback(session, tmp_path) -> None:
    catalog = MarketCatalog(session, tmp_path)
    store = CanonicalMonthlyStore(tmp_path)
    ...
    result = MarketDataService(catalog, store).query(SeriesQuery(
        "continuous", "jm", "15m", datetime(2025, 1, 2, 6, 45, tzinfo=UTC),
...
def test_actual_dominant_switches_contracts_and_uses_week_last_owner(session, tmp_path) -> None:
    catalog = MarketCatalog(session, tmp_path)
    store = CanonicalMonthlyStore(tmp_path)
    ...
    for day, contract in ((2, "JM2505"), (3, "JM2505"), (6, "JM2509"), (7, "JM2509")):
        session.add(TradingCalendar(exchange_code="DCE", trade_date=date(2025, 1, day), is_trading_day=True))
        catalog.upsert_main_contracts((("jm", date(2025, 1, day), contract),))
    session.commit()

    result = MarketDataService(catalog, store).query(SeriesQuery(
        "actual_dominant", "jm", "1d", datetime(2025, 1, 1, 7, tzinfo=UTC),
        datetime(2025, 1, 7, 7, tzinfo=UTC),
    ))

    assert [bar.close for bar in result.bars] == [Decimal("100"), Decimal("101"), Decimal("200"), Decimal("201")]
    assert [segment.contract for segment in result.resolved_contract_segments] == ["JM2505", "JM2509"]


def test_actual_dominant_fails_closed_when_rank1_map_is_incomplete(session, tmp_path) -> None:
    catalog = MarketCatalog(session, tmp_path)
    store = CanonicalMonthlyStore(tmp_path)
...
def test_actual_dominant_fails_closed_when_a_mapped_day_has_no_bar(session, tmp_path) -> None:
    catalog = MarketCatalog(session, tmp_path)
    store = CanonicalMonthlyStore(tmp_path)
    key = DatasetKey("contract", "jm", "JM2505", "1d")
    _publish(catalog, store, key, (_bar(2, 100),))
...
def test_actual_dominant_week_uses_last_trading_day_owner(session, tmp_path) -> None:
    ...
    for day in (6, 7, 8, 9, 10):
        session.add(TradingCalendar(exchange_code="DCE", trade_date=date(2025, 1, day), is_trading_day=True))
        catalog.upsert_main_contracts((("jm", date(2025, 1, day), "JM2505" if day < 8 else "JM2509"),))
    session.commit()

    result = MarketDataService(catalog, store).query(SeriesQuery(
        "actual_dominant", "jm", "1w", datetime(2025, 1, 5, tzinfo=UTC),
...
def test_query_fails_closed_for_missing_or_invalid_physical_partition(session, tmp_path, failure) -> None:
    catalog = MarketCatalog(session, tmp_path)
    store = CanonicalMonthlyStore(tmp_path)
    key = DatasetKey("continuous", "jm", "MAIN", "1d")
    ...
    with pytest.raises(MarketDataError, match="PARTITION_INTEGRITY_INVALID"):
        MarketDataService(catalog, store).query(SeriesQuery(
            "continuous", "jm", "1d", datetime(2025, 1, 1, 7, tzinfo=UTC),
            datetime(2025, 1, 2, 7, tzinfo=UTC),
        ))
