...
const reports = await Promise.all([firstReport, secondReport].map(async file =>
  JSON.parse(await readFile(file, 'utf8')),
))
if (reports.some(report => report.complete !== true || report.readonly !== true
...
async function worker(base) {
  while (next < cases.length) {
    ...
    try {
      ...
      row.reference_summary_visible = Boolean((await summary.innerText()).trim())
...
console.log(JSON.stringify({ frequency, code_sha: sha, cases: cases.length, failures, outputPath }))
