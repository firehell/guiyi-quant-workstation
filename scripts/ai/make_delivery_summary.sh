...
passed=(
    not d["failed_commands"]
    and d["scope_check"]=="passed"
    and d["forbidden_path_check"]=="passed"
    and d.get("sensitive_data_check")=="passed"
    and d["approval_valid"]
    and not d["plan_changed"]
    and d.get("issue_gate")!="failed"
    and d.get("review_status")!="high_priority_findings"
    and not d.get("external_review_required", False)
)
lines=[f"# 交付摘要 — {d['task_id']}","","## 摘要",f"- 当前状态：{d['task_status'] or '未记录'}",f"- 执行状态：{d.get('execution_status','未记录')}",f"- 工作级别：{d.get('work_level','L2')}",f"- Worktree：{d.get('worktree_path') or '未记录'}",f"- Issue Gate：{d['issue_gate']}",f"- Review：{d.get('review_status','未记录')}",f"- External Review Required：{str(d.get('external_review_required', False)).lower()}","","## 完成"]
...
lines += ["","## Warnings"] + ([f"- {x}" for x in d.get("warnings", [])] or ["- 无"])
