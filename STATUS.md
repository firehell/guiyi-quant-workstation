...
| 项目 | 当前事实 |
|---|---|
| Release | `v1.9.10@a56d13cd982cacb1f1051052e616174b55517196` 是当前最新 GitHub Release；`main`、annotated tag 的 peeled commit 与 GitHub Release target 精确一致，API 与 Web release identity 均为 `1.9.10`。这是从 `v1.9.9@c211b01e` 派生的最小 Live 因果发布热修。 |
| Runtime | v1.9.10 已创建 clean、detached 的 `/Volumes/扩展盘/guiyi-quant-runtime-v1.9.10-r1@a56d13cd`，但该 root 未预置 API `.venv` 与 Web runtime assets，五项服务启动失败并立即 fail-closed。已一次性回滚到 clean、detached 的 `/Volumes/扩展盘/guiyi-quant-runtime-v1.9.9-r1@c211b01e`；API/Web/Live/Alert 均 running，盘后服务按调度 `not_running`。未重试 v1.9.10、未执行盘后、未改 Scope/Rule、未手工发送通知；当前 Runtime 仍为 degraded。 |
...
| v1.9.10 | completed 1m Bar 仅在 atomic ready heartbeat 写入确认后才 PubSub；Redis 持久化、heartbeat、PubSub 或派生失败均 fail-closed，且同一 poll 不重试发布。完整非隔离后端、Mypy、Ruff、canonical、OpenSpec、secret scan 与 Web check/test/build 已通过，Standards/Spec 复审通过。当前为 `RELEASED`，但 Runtime 部署失败并已回滚，不能标记 `RUNTIME_READY`。 |
...
- v1.9.10 已 `RELEASED`，但 Runtime 部署因缺少 root-local Python/Web runtime assets 失败并已回滚；在新的明确部署授权前，不得重试。下次只能先以只读 preflight 验证 assets 后再切换，并重新取得首根自然 completed Live Gate。
