# 全系统审计与渐进重构决策
审计日期：2026-09-22。状态：调查、设计与任务拆分；**未实施、未 commit/push、未执行生产 mutation**。
...
1. **SA-00：收敛 develop 与已发布修正的差异。** 本地和远端 develop 相同，但 main 还有发布期修正没有回流；
   先做相关文件基线核对，完整回流可与盘后/Live小修并行；不能盲目整树覆盖。当前工程检查确有失败。
2. **SA-01 与 SA-02：并行补盘后诊断、修 Live UNKNOWN 假正常。** 今天的盘后失败已经现场证实；
   具体异常原因尚未证实，不能以“升级即修复”代替取证。Live 假正常已由现场输入与离线代码共同复现。
3. **SA-03：收口 Historical Calendar/Session 证明。** 普通 MDS 查询可接受缺中间 Calendar 的稀疏结果，
   或接受非 rqdata Session；更严格的 lifecycle 路径却拒绝相同事实。先修一条代表链，再迁移消费者。
4. **SA-04：缩短 Live 数据库事务生命周期。** 这是可证实的长期资源占用机制；不是已证实的今夜事故根因。
5. **SA-05 与 SA-06：处理主升浪合同冲突和参考仓储批次缺陷。** SA-05 等 owner 确认语义；
   SA-06 交给已有 P4 owner，先检查未提交实现是否已修，避免重复开发。
6. **SA-07/08：收敛工程验证与完成集成。** 验证按风险分层；发布、Runtime 切换、自然运行仍为独立证据。

当前不做：全仓目录搬迁、微服务、消息队列/outbox、通用重试引擎、新数据 authority、全量测试削减、
扩展 Newow 开放范围、参考交易升级成账户、Paper/OMS/Broker、策略参数优化或跨项目 AutoTrade 接入。

需要集中批准的决策在第9节；任务共享合同在任务包第2节。批准规划不等于本轮开始实施或授权生产操作。
...
- **动作**：SA-05在owner决策D2后执行。推荐恢复accepted的完整生命周期资格，暂不增加新展示语义；要接受现有按计算段放宽，
  则必须明确新合同/身份/checkpoint兼容范围。两种选择都不得创建交易或收益。
...
flowchart TD
  ...
  B --> S1[SA-01 盘后调查和诊断生命周期]
  B --> S2[SA-02 UNKNOWN与覆盖健康]
  B --> S3[SA-03 Calendar/Session证明收口]
  S2 --> S4[SA-04 Live事务边界]
  S3 --> S4
  D2[Owner D2 主升浪语义] --> S5[SA-05 合同与身份对齐]
  S0 --> S6[SA-06 原P4 owner批次接缝]
  S5 --> S6
  S0 --> S7[SA-07 工程验证与文档减负]
  S0 --> S8[SA-08 集成与运行验收准备]
  S1 --> S8
  S4 --> S8
  S6 --> S8
  S7 --> S8
...
SA-00不是小修实施的硬前置：main差异无Live/after_market/runtime_health，相关文件基线核对后01/02可与回流并行。
SA-01只读取证可立即进行；01和02可并行；SA-03在不同时改composition的安排下并行其纯事实层。
SA-04等02/03公共状态稳定后再改Live/composition。
SA-06的P3缺陷可先复现，涉及Newow资格/checkpoint的迁移等SA-05。
SA-07的公共根文档统一owner，不能每个任务同时改STATUS/TESTING/DECISIONS。

推荐合并序：00 → 02 → 01 → 03 → 04 → 05 → 06 → 07 → 08；
不要求等所有重构完成才交付01/02的小修。
主升浪决策未批准时，01–04、07可独立继续，SA-08可形成仅稳定性修复候选。
每次合并核对develop相对审计SHA的diff、main回流与P4状态；计划不自动适用于新合同。
...
| 决策 | 推荐 | 其他选项与后果 |
|---|---|---|
| D1：架构范围 | 选方案B：先修已证实故障，再收口事实证明与事务；保持单体和已有authority | 方案A只修点可先交付但重复判断保留；方案C全重分层目前无充分收益 |
| D2：主升浪无入场CLEAR | 恢复accepted的完整lifecycle资格，暂不增加中断后新资格 | 另一选项是接受按计算段放宽，正式改合同/版本/checkpoint兼容；影响展示范围，不能让Sol自行选择 |
| D3：交付收敛与工程约束 | 发布修正必须回流develop；稳定版本做一致性验证，必要计划可保留在现有目录，去掉每次手写文件名清单的机械负担 | 保留精确文件清单需每次计划/证据增加都同步，继续形成重复维护；不建议 |
| D4：实施范围 | 先授权00–04、07与08的工程准备；05待D2，06由原P4负责人结合已授权工作接手 | 可以只批准00/01/02先止血；大重构/生产修复仍不纳入 |

共享数据完整性、UNKNOWN诚实降级、event先commit/one-shot、锁事务与auto_order=false保持不变。
以上是待批准建议，不改accepted canonical；批准实现也不含下载、Canonical/生产DB/Redis写、
真实通知、main/tag/Release或Runtime promotion。
