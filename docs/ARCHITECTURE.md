...
| S6-08 SignalEvent | 旧 JM V1-B schema-v2 代码与 packet 仅作 superseded 历史；HTDY Step 3 immutable writer/完整 lineage v2/Stage 9 preview-only 例外已完成，delivery 与通知仍禁止；code-only Runtime 已部署到 `22760122`，但 rebind 因 checkpoint schema 查询错误在 receipt 前 fail-closed。修复 checkpoint、新三包/批准、rebind receipt 与真实事件仍 pending |
