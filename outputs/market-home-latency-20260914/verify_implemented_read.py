...
try:
    ...
    with SessionLocal() as session:
        ...
        for label, cls in [('baseline', module.MarketDataService), ('implemented', candidate)]:
        ...
        same = results[0]['response_sha256'] == results[1]['response_sha256']
        assert same, 'response mismatch'
        session.rollback()
    (OUT / 'implemented-read.json').write_text(json.dumps({'checked_at': datetime.datetime.now().astimezone().isoformat(), 'transaction': 'repeatable read/read only', 'baseline': BASE, 'same_response': same, 'runs': results}, indent=2))
