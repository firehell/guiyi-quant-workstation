...
### 3.1 Release、数据 apply、Runtime、自然 Event 是四类不同事实
...
当前 production 已在 Alembic `20260903_0045`，warm-up repair 不新增 migration。因此除非 fresh RC diff 重新出现 migration 变化，否则本 Gate 不要求连接 production PostgreSQL，也不要求为了 release 人为运行 isolated PostgreSQL。
...
```text
version = v1.9.15
rc_head_sha = <40-char reviewed RC SHA>
rc_tree_sha = <reviewed tree SHA>
PR = #333
current main SHA = <fresh SHA>
```
...
```text
release commit tree == reviewed RC tree
```
...
- release tree 与 reviewed RC tree；
...
- 如果 `2026-09-03` 仍足以满足 exact-tag Runtime 的同 physical-contract replay，则保持该目标；
- 如果 Runtime 要求更晚的完整交易日前缀，则生成新的明确 `through=<exact date>` plan，向 Owner 报告范围变化，并以新的 `plan_sha256` 作为下一 Gate 唯一授权对象；
...
没有稳定 hash 或 identity 不完整时，不得进入 apply。
...
- Rule、Scope、Event 数、notification 状态未变化。
...
release tag = v1.9.15
release commit = <exact peeled commit>
services = API / Web / Live / after-market / Alert
new runtime root = <exact detached path>
rollback root = <如需要，唯一预先指定旧 root>
```
...
1. 从 exact tag 建立/确认 clean detached runtime root；
2. render-only；
3. 切换五项 launchd 服务；
4. readback 已加载 `GUIYI_PROJECT_ROOT` 与 `GUIYI_RUNTIME_COMMIT`；
5. 五项服务必须指向同一 exact release identity。
...
- 不改 Scope；
- 不补发通知；
- 仅当本次授权预先包含唯一 rollback root 时，允许一次 rollback；
...
series_kind = actual_dominant
completed = true
runtime_commit = exact v1.9.15 release commit
```
...
5. `main`、annotated `v1.9.15` tag、GitHub Release identity 一致；
...
11. exact `v1.9.15` Runtime 出现自然 completed SuBing 15m Candidate；
12. 同一 Candidate 先形成 immutable `AlertEvent`；
13. 同一 Event 获得一次 PushPlus provider acceptance；
14. Owner 人工确认微信实际收到同一自然 Event；
15. `STATUS.md` 与 Issue `#307` 按真实事实收口，且没有 synthetic/replay/manual send 代替自然证据。

只有第 1–15 项全部满足，最终结论才允许为：
