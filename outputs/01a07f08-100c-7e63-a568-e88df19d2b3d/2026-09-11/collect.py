...
def safe(exc):
 ...
 if code is None and exc.args and isinstance(exc.args[0],str) and re.fullmatch('[A-Z0-9_]+',exc.args[0]):code=exc.args[0]
...
def main():
 assert not (OUT/'source.json').exists(),'SNAPSHOT_ALREADY_EXISTS'
 source={'cutoff':NOW,'today':TODAY,'week_start':MON,'runtime_roots':roots,'loaded':loaded,'runtime_head':subprocess.check_output(['git','-C',str(ROOT),'rev-parse','HEAD'],text=True).strip()}
