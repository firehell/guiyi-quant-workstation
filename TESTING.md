...
这组测试只使用临时目录/fake service，验证 projection identity、atomic file、API projection-hit/miss、`data update/refresh --apply` 失效和 after-market 顺序；不得以测试为理由执行真实 `guiyi data ... --apply`。真实 projection-hit 性能 `<200ms` 属于后续明确授权的本地 Runtime read-only manual acceptance，不在普通 pytest 中用 timing sleep 伪造。
