"""Read-only formal PX W1 API smoke on development code and actual Catalog."""
...
try:
    with TestClient(app) as client:
        ...
        if (cap.status_code != 200
                or capability["schema_version"] != "newow_product_capabilities_v19"
                or len(capability["weekly_products"]) != 57
                or "px" not in capability["weekly_products"]
        ...
        for strategy, statuses in expected.items():
            for section, expected_status in statuses.items():
                params = {
                    "product": "px", "strategy": strategy, "frequency": "1w", "section": section,
        ...
        closed = client.get(
            ...
            params={"product": "sf", "strategy": "trend", "frequency": "1w", "section": "chart"},
        )
        ...
        result = {"status": "passed", "scope_count": 57, "schema_version": capability["schema_version"],
                  "sections": sections, "sf_still_closed": True, "database_mode": "read_only"}
        (Path(__file__).parent / "px-formal-scope-smoke.json").write_text(
