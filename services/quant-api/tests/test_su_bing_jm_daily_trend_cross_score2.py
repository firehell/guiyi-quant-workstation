...
STRATEGY_DIR = QUANT_CORE_ROOT / "guiyi_quant" / "strategies" / "su_bing_jm_daily_trend_cross_score2"
...
def _make_strategy(**overrides):
    from guiyi_quant.strategies.su_bing_jm_daily_trend_cross_score2 import SuBingJmDailyTrendCrossScore2Strategy

    return SuBingJmDailyTrendCrossScore2Strategy(None, "jm-trend-cross-test", "jm2405.DCE", _setting(**overrides))
...
def _snapshot(
    *,
    ...
    from guiyi_quant.strategies.su_bing_jm_daily_trend_cross_score2 import IndicatorSnapshot
...
def test_default_params_identify_independent_trend_cross_version() -> None:
    from guiyi_quant.strategies.su_bing_jm_daily_trend_cross_score2 import STRATEGY_CLASS_PATH, validate_params
    ...
    assert STRATEGY_CLASS_PATH.endswith("SuBingJmDailyTrendCrossScore2Strategy")
    assert params.strategy_code == "su_bing_jm_daily_ema21_macd_volume"
    assert params.strategy_version == "v0.3.1-daily-trend-cross-score2"
    assert params.min_entry_score == 2
    assert params.macd_zero_threshold == 25
    assert params.require_trend_alignment is True
    assert params.require_macd_cross is True
    assert params.volume_rule == "current_volume_gt_previous_volume"
    assert params.submit_vnpy_orders is False


def test_trend_and_matching_macd_cross_are_required_even_when_score_is_two() -> None:
    from guiyi_quant.strategies.su_bing_jm_daily_trend_cross_score2 import evaluate_trend_cross_score2_signal, validate_params

    params = validate_params()
    accepted = evaluate_trend_cross_score2_signal(
        _snapshot(close=112, current_volume=1000, previous_volume=1000),
        params,
    )
    trend_plus_volume = evaluate_trend_cross_score2_signal(
        _snapshot(
            close=112,
            dif=80,
            dea=70,
    ...
    cross_without_trend = evaluate_trend_cross_score2_signal(_snapshot(close=100), params)

    assert accepted.direction == "long"
    assert accepted.entry_score == 3
    assert accepted.satisfied_conditions == ["long_trend_ok", "macd_near_zero", "long_macd_cross"]
    assert accepted.directional_anchor == "long_trend_ok+long_macd_cross"
    assert trend_plus_volume.direction == "none"
    assert trend_plus_volume.rejected_reason == "macd_cross_required"
    assert trend_plus_volume.entry_score == 2
    assert cross_without_trend.direction == "none"
    assert cross_without_trend.rejected_reason == "trend_alignment_required"
    assert cross_without_trend.entry_score == 3
...
def test_strategy_trade_records_score_tags_and_v02_exit_logic() -> None:
    ...
    assert trade["trade_id"].startswith("SB-JM-TC-D-")
    assert trade["strategy_version"] == "v0.3.1-daily-trend-cross-score2"
    ...
    assert "trend_cross_confirmed" in trade["scene_tags"]
...
def test_future_bars_and_review_tags_are_not_signal_inputs() -> None:
    ...
    signal_source = inspect.getsource(evaluate_trend_cross_score2_signal)
    on_bar_source = inspect.getsource(SuBingJmDailyTrendCrossScore2Strategy.on_bar)
