...
本表把页面观察、手册主张、归一实现和期货迁移证据分开。`OBSERVED_EXACT` 只代表公开页面/源码事实，`REPRODUCED_EXACT` 才代表已有逐值复算；`INFERRED_CANDIDATE` 不得晋升正式策略；`UNKNOWN` 与 `OUT_OF_SCOPE_PRIVATE` 均没有实现入口。

外部证据根：`newow-strategy-detail-research/v3.2.82-gap-closure`  
证据清单 SHA-256：`f0b4fec68c90a6b2f9fed73e731d6508f49cfe7a0e6de9fa994c977c46b395dc`
...
| 杯柄形态 primitive | 手册加详情页 v3.2.63 | INFERRED_CANDIDATE | newow_cup_handle_v1 | packages/quant-core/guiyi_quant/newow/cup_handle.py | 股票形态样本和 causality 测试已有 | actual_dominant D1 研究 primitive 已有 | 不能宣称当前服务端 cup_handle 筛选 exact |
...
| 参数比较器因果研究口径 | 归一设计 v1 | INFERRED_CANDIDATE | newow_channel_window_compare_causal_v1 | none | 不适用 | 需要 next-open、成本、rollover、prefix 合同 | Slice B TDD 实现并保留研究边界 |
...
| 账户、自选、盯盘、订阅与分享 | 需用户私有状态 | OUT_OF_SCOPE_PRIVATE | none | none | 未采集且不触发 | 不适用 | 永久边界，除非用户另行明确授权 |
| 基本面、CANSLIM 与大师选股 | screener.html public UI only | OUT_OF_SCOPE_PRIVATE | none | none | 仅有页面阈值描述，无权威数据源合同 | 不扩建 A 股基本面平台 | 保留文档研究，不实现 |
...
- 页面浏览器实际使用跨域公开 API；本 Slice 没有扩大已批准的网络监听 host。27 个同 host K 线响应是稍后独立交叉核对，不冒充截图页面当时的原始字节。
