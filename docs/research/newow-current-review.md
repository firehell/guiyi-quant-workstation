...
当前浏览器可见内容没有提供可导出的完整策略输入及未舍入参考价；本轮未读取私有服务端公式、未解锁付费报告、未下载RQData或连接生产DB。要关闭新版逐值parity，需要至少固定一个标的/策略/周期的同一截止快照：完整warm-up和OHLCV、复权/价格口径、精确BUILD/CLEAR及关联身份、未舍入entry/exit和目标/吸筹输入。取得后才能让现有纯函数消费同一输入比较；不同日期的截图或期货fixture无法替代。
