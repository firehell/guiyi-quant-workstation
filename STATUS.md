...
- SuBing 全历史效果的真实 schema-v2 采纳、active60 Git 外 cache warm 或增量刷新、以及含该派生阶段的 Runtime promotion，仍须分别取得新的单次明确授权；仓库 `CODE_COMPLETE` / `TEST_COMPLETE` 不自动授予任一 Gate，也不声明 `RELEASED` 或 `RUNTIME_READY`。
