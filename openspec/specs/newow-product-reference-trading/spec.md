...
全部 60 品种开放 `1d`，并对版本化 51 品种开放 `1w` 的
chart/auxiliary/reference/comparator；其余 9 品种的 `1w` 返回
...
正式日周版使用 `newow_product_capabilities_v13`、`daily_weekly` 声明 `1d/1w`，并携带唯一的
`weekly_products`。b、bz、cj、eb、eg、j、oi、pg、si、sr 在正式周线请求上使用 `newow_weekly_input_quality_v2`，
...
其中尚未正式开放的 9 个品种仍只在候选请求中进入 reader，正式请求返回
`NEWOW_PRODUCT_FREQUENCY_NOT_OPEN`。
正式 v13 合同只有在独立 Release 与 Runtime promotion 完成后才成为现场事实。历史 v12、v11、v10 与 v8
分别表示先前的 50、49、48 与 41 品种正式周线。
