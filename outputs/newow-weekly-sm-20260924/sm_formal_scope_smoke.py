"""Read-only formal SM W1 API smoke on development code and actual Catalog."""
...
try:
    with TestClient(app) as client:
        ...
        if (cap.status_code != 200
                or capability["schema_version"] != "newow_product_capabilities_v22"
                or len(capability["weekly_products"]) != 60
                or "sm" not in capability["weekly_products"]
                or len(set(capability["weekly_products"])) != 60):
        ...
        for strategy, statuses in expected.items():
            for section, expected_status in statuses.items():
                params = {
                    "product": "sm", "strategy": strategy, "frequency": "1w", "section": section,
        ...
        result = {"status": "passed", "scope_count": 60, "schema_version": capability["schema_version"],
                  "sections": sections, "database_mode": "read_only"}
        (Path(__file__).parent / "sm-formal-scope-smoke.json").write_text(
