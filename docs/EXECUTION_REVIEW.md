...
- Alert 先独立提交 Event；Execution Review 故障不得反向阻断 Alert 或 WeCom one-shot 路径。
...
- 历史 NULL snapshot enrichment 若未来需要，必须作为独立 Lane 3 设计和人工 Gate。active-60 60/60 completion 也是独立 reference-data 目标，不是 v1.4 release Gate。
...
代码与测试不授权 release、production migration、Runtime switch、roll marker、真实通知、Scope 或订单。v1.4 release、生产 DB `20260815_0039` migration、五服务 Runtime promotion 与 roll marker activation 是四个独立人工 Gate；当前执行状态只看 `STATUS.md`。
