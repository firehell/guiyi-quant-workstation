...
- [x] 同一 reviewer 审查完整 release diff；Critical/Important 必须为 0。
...
- [x] Release PR 合入 `main`，创建并 push annotated `v1.6.4`，读回 tag object、peeled commit、`origin/main`；再同步 develop。
- [x] 创建 clean/detached exact-tag Runtime，锁定依赖、build、test、render-only，再单次执行 base -> Market -> Alert switch。
- [x] 读回五服务 exact root/commit、API/Web/Runtime、DB head、60 品种、两个 exact Scope、PushPlus transport 和无意外 notification attempt。

### Task 6: Fail-Closed Cleanup and Closeout

- [x] 删除前扫描 launchd/plist/PID/open files，确认无正式服务引用目标根。
- [x] 仅用 `git worktree remove` 删除 v1.6.3 release worktree、v1.6.2 Runtime 和本次临时 worktree；保留 v1.6.3 Runtime 回滚源。
- [x] 更新 STATUS，push develop，并读回最终 remote SHA；自然 SuBing/HTDY/after-market 验收保持真实 pending 状态。
