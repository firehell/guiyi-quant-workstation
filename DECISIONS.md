...
| Alert Runtime V2 授权 | 已部署 Alert v1.6.4 的持续边界为 `htdy_original_15m × 该 Rule 显式 scope_products × htdy_observers × pushplus-wechat-topic` 与 `subing_entry_signal_v1 × 该 Rule 显式 scope_products × owner × pushplus-wechat`；当前两条 Scope 均精确为 `jm` | 已批准 Topic 可在人工核对的 `1..4` 人边界内增加成员，超过 4 人、未知成员或换 Topic 须重新授权；未来第三条 Rule、production migration、后续 release/tag、再次 Runtime switch、Scope/transport 变化、真实 canary/send 与 rollback 互不授权 |
