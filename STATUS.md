...
- Git 外 private config 已按单次授权原子写入并只读验证：parent=`0700`、file=`0600`、current uid、
  ...
  `network_called=false / would_send=false`。未发送通知，未连接 production DB，未执行 Scope、
  main/release/tag、Runtime promotion/switch 或外部旧配置清理。
- 专用 Topic 已创建且当前 3 人由用户确认；第 4 人允许后续加入。独立 pending Gate 为：分别执行
  `owner` 与 `htdy_observers` 真实 canary、取得精确 Rule + Scope +
...
- production 事实保持不变：仍为
  ...
  仍精确为 `jm`。没有 PushPlus 已发送、已 release/promotion 或已自然验收的证据。
...
- 最小下一步：保持 production `v1.6.2 + clawbot-openclaw-weixin` 不变；单独批准并先执行一次
  `owner` 真实 canary，再另行决定 HTDY Topic canary。
