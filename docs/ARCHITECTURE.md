...
HTDY supports all operational products and seven frequencies. Its current event identity and authorization are frequency-aware. SuBing remains product-scoped. Alert and Execution Review are independent Application Domains and do not alter the Market Catalog.

## Research and Runtime

JDJ reference replay and RQAlpha are separate research-only paths. Candidate Validation/Robustness retain strict causal and prospective OOS boundaries. RQAlpha is an unmounted loopback local app.
